...
# 商品は「教材PDF 36冊」と「写経用の土台（この ZIP）」の2点で、読者は PDF を
# 見ながらこの ZIP へ写経する。だからここに梱包するのは、ZIP 展開後に
# scaffold-from-scratch.sh が土台を組み立てるのに要るものだけである。
...
# material/ は ZIP に入れない。読者は PDF を見ながらこの土台へ写経するので、
# 同じ内容の原稿 Markdown と、PDF へ焼き込み済みの画面写真は使い道が無い。
# 集めた後に外すのではなく最初から集めん。build ディレクトリに置かなければ
# 混入の経路そのものが無くなる。
...
FILE_COUNT=$(find "${BUILD_DIR}" -type f ! -name ".DS_Store" | wc -l | tr -d " ")
echo ""
echo "=== ビルド完了 ==="
