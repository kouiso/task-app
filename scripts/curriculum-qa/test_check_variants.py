...
CASES: list[tuple[str, str, int]] = [
    ...
    # 4連で開いたブロックを3連では閉じない。閉じたことにすると、後続のコードが
    # 地の文として検査され、実装の文言を書き換えろと報告してしまう。
    (
        "4連開始・3連終了では閉じない",
        "````md\n```\n既に\n```\n既に\n````\nすでに\n",
        0,
    ...
    # 2連の code span も実装の文言の引用なので直さない。
    ("2連バッククォートの中は直さない", "| ``このユーザーは既にメンバーです`` | 追加済み |", 0),
    ("2連バッククォートの外は拾う", "``既に`` の話ですが、既にある場合は上書きします。", 1),
]
