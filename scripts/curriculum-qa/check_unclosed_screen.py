...
実測（2026-08-31 時点の36本）で、この形の場所は5つあった。いちばん分かりやすいのが
day10 で、`src/component/project/project-dialog.tsx` の絵を Step 5 の末尾に置いている。
その時点で `</Dialog>` は書かれておらず（Step 6）、画面へ組み込むのも先（Step 7）である。
5つとも教材の側が「この時点ではまだ画面に出せません」と断りを入れており、断りを
...
途中の状態を判定する足場が無い。現物では7つが該当し、全部この理由で対象外になる。

## この検査で捕まえられないもの

- 対象外にした7つの書き込み先。そこで同じ欠陥が起きても鳴らない。足場が無いので
