...
## 期待値が空のケースのうち3つは、何を壊しても落ちない

「文字列とコメントの中のかっこは数えない」「JSX を書けない .ts の `<Foo>` は開始タグに
数えない」「自己終了タグは開いたままにしない」の3つは、期待値が空である。この3つは
`judgeable_targets` に自己中和される。`mask_code` や `allows_jsx` を壊すと、その書き込み先の
30日ぶんの収支まで一緒に崩れる。収支が合わない書き込み先は判定対象から外れるので、
壊しても結果は空のまま＝テストは緑になる。変異を入れて実測して確かめた。

残す理由は、同じ3つが `test_check_tag_balance.py` の「コメントの中の閉じタグ」
「.ts の型アサーション」「自己終了タグ」で落ちること。あちらは `CORPUS_CHECKS` にも
Gate 4 にも載っているので、ゲートとしては穴が開いていない。この3つはそこの再掲であり、
ここで守っているつもりになってはいけない。
"""
