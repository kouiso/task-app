...
# 使用例: bash scripts/generate-pdf.sh single /path/to/file.md 1
#         bash scripts/generate-pdf.sh all 1
...
generate_single_pdf() {
  ...
  if [ -z "$file" ]; then
    ...
    echo "使用法: bash scripts/generate-pdf.sh single docs/file.md [OPEN]"
    exit 1
  fi
