...
|---------|-----------|-----------|---------|------|------------|
| 1 | task | 9 | 13, 14, 15, 16, 28（task.getById は comment 族の day18 で追記） | register-only / Step0なし | project (day09-12) |
...
| 5 | user | 5 | 24, 25, 29 | register-only / Step0なし | project |
...
3. **scaffold が task.ts / search.ts を完成品として配布したまま**: `scaffold-from-scratch.sh` は現状 project.ts のみ配布から除外しており、task.ts / search.ts は完成ファイルとして受講生の手元に配ってしまう。
   解決: この PR (day13/14 の写経化) は、wave-1 の「task.ts / search.ts を配布対象から外す SBI」と同時に、またはその直前にマージする。単独で受講生へ出すと、写経させたいファイルが最初から完成品として置かれてしまうため、単独マージは禁止。
