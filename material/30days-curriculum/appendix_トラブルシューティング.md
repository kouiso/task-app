...
| エラー | 原因 | 解決方法 |
|--------|------|----------|
| ログインできない | Day 25 でパスワードを変更したかユーザーが存在しない | 変更後のパスワードを使います。ユーザーごと消えている場合だけ `npm run db:seed` を実行します（すでにいるユーザーのパスワードは入れ直されません） |
...
> `npm run db:seed` を実行する前に読んでください。このコマンドは初期データの2つのプロジェクトをいったん削除して作り直します。そのプロジェクトに自分で追加したタスクとコメントも一緒に消えます。すでにいるユーザーのパスワードは入れ直されません。
