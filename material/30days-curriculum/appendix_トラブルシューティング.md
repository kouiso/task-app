...
| エラー | 原因 | 解決方法 |
|--------|------|----------|
| `node: command not found` | Node.jsがインストールされていない | Day 01のOS別手順でNode.js 22系をインストール。mise利用時は`task-app`フォルダで`mise trust && mise install`を実行 |
| `npm install`でエラー | Node.jsのバージョンが教材と異なる可能性がある | `node -v`で確認します。22系でなければDay 01のOS別手順で22系を使う設定に変更。mise利用時は`mise trust && mise install`を実行 |
...
| `環境変数の検証に失敗しました` | `.env` に必要な変数がないか `JWT_SECRET` が32文字未満 | エラー本文に出ている変数名を `.env.example` と見比べて `.env` に追記します。Vercel では Project Settings の Environment Variables に `DATABASE_URL` と `JWT_SECRET` を登録します |

### Day 05-08: 認証・ログイン

| エラー | 原因 | 解決方法 |
|--------|------|----------|
| ログインできない | Day 25 でパスワードを変更したかユーザーが存在しない | 変更後のパスワードを使います。ユーザーごと消えている場合だけ `npm run db:seed` を実行します（初期データの2プロジェクトとその中のタスク・コメントは作り直され、自分で追加した内容は消えます。すでにいるユーザーのパスワードは入れ直されません） |
...
| エラー | 原因 | 解決方法 |
|--------|------|----------|
| プロジェクト一覧が空 | ログインユーザーがメンバーのプロジェクトがない | 新規プロジェクトを作成します。`npm run db:seed` は上の注意を読んでから実行してください |
| メンバー追加できない | OWNER/ADMIN権限がない | 自分の権限を確認します。権限がなければプロジェクトオーナーに依頼します |
...
| ステータス変更が反映されない | `onSuccess` での `invalidate()` 呼び忘れによって一覧の控えが古いまま | ステータス変更の `useMutation` の `onSuccess` に `utils.task.getAll.invalidate()` があるか確認します。リロード（Windows と Linux は Ctrl+Shift+R、Mac は Cmd+Shift+R）で直る場合もこの呼び忘れを確認します |
...
| レポートにデータが表示されない | 完了タスクがないか期間外 | タスクを完了にして期間設定を確認 |
...
**解決**: 開発サーバーは空いている番号で起動します。このメッセージに出た番号をブラウザで開いてください。3000番のまま使いたい場合は`lsof -i :3000`で使用中のプロセスを確認します。そのプロセスを終了してから起動し直します。
