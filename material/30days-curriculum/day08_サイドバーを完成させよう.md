...
Day 07 ではログイン成功時のトースト通知を実装し、JWT認証・bcryptパスワード検証・HttpOnly Cookieの仕組みを学びました。認証の基盤が整ったので、今日はサイドバーにユーザー情報表示とログアウト機能を追加します。
...
| ファイル | 意味 | 状態 |
|---------|------|------|
| `src/component/ui/alert-dialog.tsx` | shadcn/uiのAlertDialogコンポーネント | 既にプロジェクトに含まれている |

このファイル: `src/component/ui/alert-dialog.tsx`
...
Step 4で追加したユーザー情報ウィジェットの下に、ログアウトボタンを追加します。まずログアウト処理を定義します。
...
次に、AlertDialogで確認付きのログアウトボタンを作ります。
...
Step 5 で実装した `logoutMutation` と `handleLogout` が連携して動きます。

| 処理フロー | 担当コード | 例え |
|-----------|-----------|------|
| ① ボタンクリック → 確認ダイアログ表示 | `AlertDialogTrigger` | ドアノブを握る |
| ② 「ログアウト」をクリック | `AlertDialogAction` → `handleLogout` | 「退出します」と宣言 |
| ③ API呼び出し → Cookie削除 | `logoutMutation.mutate()` | 退出処理 |
| ④ ログイン画面へ遷移 | `router.push('/login')` | 受付に戻される |
