...
`logoutMutation` を先に用意しておくのは、ボタンが押された瞬間に通信の準備から始めていては間に合わないためです。`onSuccess` で `router.push` に加えて `router.refresh()` を呼ぶのは、サーバー側で組み立て済みの表示が残るのを防ぐためです。これを省くと、ログアウトしたのに前のユーザー名が画面に残る場合があります。
