...
> `--font-inter` などの変数名は`src/app/layout.tsx` にある `Inter({ variable: '--font-inter' })` などの `variable` 指定で決めています。Day 01 で書いた `globals.css` がこの変数を参照しているので消すとフォント指定が黙って効かなくなります。
>
...
| 順番 | 処理 |
|---|---|
| 1 | `cancelQueries()` で実行中の問い合わせを止める |
| 2 | `clear()` で、前の利用者の名前やタスクをブラウザ内のキャッシュ（取得済みデータを一時保存する場所）から消す |
| 3 | ログイン画面へ移動し、`router.refresh()` でサーバー側の表示を作り直す |
...
読み込み中の表示をここでは自分で書いています。Day 09 で別のページの読み込み中表示を作るときは、配布済みの `PageLoadingSpinner` を使います。今日の `AppLayout` は仕組みを追いやすいように素の要素のまま完成させます。
...
図のいちばん上にある `src/app/layout.tsx` はすべてのページの親なのでログイン画面と登録画面もここを通ります。一方 `AppLayout` はダッシュボードの内側にだけ置きます。サイドバーを出したい画面と出したくない画面はこの置き場所だけで分かれます。
...
今日は4つのファイルを扱いました。このうち `providers.tsx` と `layout.tsx` は中身の確認だけです。断片を貼り重ねる作業が続いたので途中でどこへ貼ったか分からなくなった場合は以下のコードと自分のファイルを見比べてください。上から順に読めばStep 1 から Step 4 で扱ったものがどう1つのファイルになったかを確かめられます。
...
ここが今日のいちばんの要点です。2つの `if` を `return` より前に置いてあるのでこの行より下は「ログイン済みの人だけが通る場所」になります。だから後ろのサイドバーでは `session?.user?.name` と書かずに `session.user.name` と書けます。ただし名前そのものは未設定があり得るので頭文字を取り出す `?.[0] || 'U'` は残します。判定を後ろへ動かすと未ログインの人にサイドバーが一瞬見えてしまいます。`return null` は何も描かずに終わる書き方で、`/login` への移動が終わるまでの短い間だけ働きます。
...
`<nav>` に付けた `flex-1` が余った高さをこの区画に全部渡します。だから下のユーザー情報はメニューの件数が変わってもサイドバーの底に貼り付きます。`cn` に渡す2つ目の値は開いているページかどうかで切り替わります。この切り替えが無いとユーザーは自分がどこにいるかを画面から判断できません。
...
`{children}` は、この枠で囲んだページの中身が入る場所です。`overflow-y-auto` を本文側だけに付けてあるので縦に長いページでもサイドバーは動かず、本文だけがスクロールします。ページ側が `<main>` を持たないのはこの行がすでに1つ持っているからです。
...
| | |
|---|---|
| タイトル | Day 08: サイドバー付きのアプリレイアウトを作ろう |
| 著者 | 磯貝光佑 |
| 版 | 第1版（2026年9月19日） |
