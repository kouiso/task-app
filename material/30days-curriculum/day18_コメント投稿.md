...
逆に import だけ書いて登録を忘れると、エラーは1つも出ないのに `api.comment` が存在しないままになります。
...
これが無いと、まだタスクを開いていない状態でも空文字の `id` が送られ、毎回 `NOT_FOUND` になります。
...
`taskDetail.comments?.length ?? 0` は、2段構えで「まだ数えられない状態」を吸収しています。
`comments` が `undefined` のとき、`?.` は `length` を読まずに `undefined` へ進みます。
`?? 0` は、その `undefined` を受け取って `0` に置き換えます。
どちらも無いと、読み込みが終わる前の一瞬で `undefined` から `length` を読もうとしてエラーになります。
...
`=== 0` と厳密に比べている理由は、`undefined` のときにこの案内を出さないためです。
読み込み中は `comments` がまだ `undefined` なので、`length` も `undefined` になります。
`undefined === 0` は `false` になるため、案内は出ません。
ここを `!taskDetail.comments?.length` と書くと、読み込み中にも「コメントはまだありません」が一瞬だけ表示されます。
本当は 3 件あるのに「ありません」と出て、その後で中身が現れます。
読み手は一瞬混乱します。
...
`name` が入っていればそれを表示します。
空なら `email`、それも無ければ `?` になります。
順番をそろえてあるので、アイコンの頭文字が「T」なのに名前が別人、という食い違いは起きません。
`new Date(comment.createdAt)` でいったん `Date` へ包み直します。
tRPC の返り値は文字列になる場合があるからです。
`format` は `Date` を受け取る前提なので、包まずに渡すと実行時エラーになります。
...
ここで取り込む4つは役割が分かれています。
`useForm` は入力値と送信を管理します。
`zodResolver` は zod スキーマと react-hook-form をつなぐ通訳です。
`z` は Step 0 の server 側で使ったのと同じ道具で、画面側にも同じ形のルールを書けます。
`Textarea` は複数行を書ける入力欄で、改行を含むコメントを受け取れます。
1行だけの `Input` にすると、長い経過報告を書きたい人がすぐ困ります。
Day 14 のログインフォームで使った道具立てをそのまま持ってきているため、新しく覚えることはほとんどありません。
...
中身は2つです。
`utils.task.getById.invalidate({ id: taskId })` がコメント一覧を取り直させます。
`commentForm.reset()` が入力欄を空へ戻します。
...
`invalidate` は、キャッシュに「この中身はもう古い」と札を貼る操作です。
札を貼られたクエリのうち、いま画面で使われているものは即座に取り直されます。
`TaskDetailDialog` を開いたままなので、`task.getById` はその場でもう一度サーバーへ問い合わせます。
返ってきた配列で `taskDetail` が差し替わります。
`.map` は描き直され、投稿した1件が最上部へ現れます。
`orderBy: { createdAt: 'desc' }` で新しい順に並べてあるため、書いたコメントを探し回らずに済みます。
...
スクリーンショット: コメント投稿後にリストが更新された画面の表示を確認してください。
