...
`...(startDate && { startDate })` という書き方は、日付が入っているときだけその項目を作ります。`startDate: undefined` と書いてもエラーにはなりませんが、その場合はキーだけが残ります。Day 11 で決めたとおり、送るデータは「日付が入っているか、項目が無いか」のどちらかにそろえます。
