...
今回は詳細を別ルートに完全分離するのではなく、一覧ページの延長として表示を切り替える構成にします。こうすると、
...
🎯 **ゴール**: `project.archive` と `project.unarchive` で `isArchived` を切り替えられるようにする。
...
🎯 **ゴール**: モーダルではなく、ページ内に表示する詳細ビューコンポーネントを作る。
...
🎯 **ゴール**: `ProjectDetailView` の中に、現在の完成形と同じ情報量を持つ 2 つのカードを配置する。
...
🎯 **ゴール**: 詳細画面上部のボタンで `archive` / `unarchive` を切り替えられるようにする。
