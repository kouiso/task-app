...
> 💡 このブロックはダイアログ全体の構造（ヘッダー・フッター・閉じるボタン）をまとめて定義しています。各パーツの役割を1つずつ確認しながら写してみてください。
...
> 💡 以下のコードブロックはアバター部分まで（ここで一旦区切り）です。この次のブロックで名前・ロールバッジ・削除ボタンを追加して `.map()` を閉じます。2つのブロックを合わせて1つのまとまりになります。
...
まず、タスク一覧セクションの見出しと `.map()` の外枠を書きます。
...
続けて、各タスクカードの中身（タイトルとバッジ）を書き、`.map()` と外側の `<div>` を閉じます。
