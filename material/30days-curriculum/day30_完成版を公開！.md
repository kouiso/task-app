...
入れ子が3段になっている理由は、この形が「どのURLに、どのヘッダーを配るか」の組を何通りでも書けるようにしてあるからです。いちばん外の配列が組の一覧、その中の `source` が対象のURL、内側の `headers` が配る中身です。今回は組が1つしかないので冗長に見えますが、あとから管理画面だけ別の指示を配りたくなったときに、2つ目の組を並べるだけで済みます。`async` が付いているのは、Next.js がこの関数を `await` で呼ぶからです。待てる形にしておけば、あとから外部の設定を読みに行くような時間のかかる処理を足せます。`async` を外して配列をそのまま返しても読み取ってもらえるので、書き忘れてビルドが止まることはありません。
