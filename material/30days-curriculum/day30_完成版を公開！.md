...
【スクリーンショット】デプロイ先で開いたログイン画面は、手元で見ていたものと同じです。
下の画像は Day 05 で作ったログイン画面を手元で撮ったものですが、デプロイ先でも同じ見た目になります。
公開しても画面が変わらないのは、同じコードが動いているからです。
...
入れ子が3段になっている理由は、この形が「どのURLに、どのヘッダーを配るか」の組を何通りでも書けるようにしてあるからです。いちばん外の配列が組の一覧、その中の `source` が対象のURL、内側の `headers` が配る中身です。今回は組が1つしかないので冗長に見えますが、あとから管理画面だけ別の指示を配りたくなったときに、2つ目の組を並べるだけで済みます。`async` は無くても動きます。配列をそのまま返すだけなら、外しても読み取ってもらえます。付けるのは、関数の中で `await` を使う処理を足したときです。外部の設定を読みに行くような時間のかかる処理を後から入れられるよう、はじめから付けてあります。
...
**Q1. `open https://your-app.vercel.app` でアプリが開けました。これで「インターネット越しに届いている」と言えますか。言えないなら、いちばん早い確かめ方も答えてください。**

A. 言えません。`open` は自分のパソコンのブラウザを開くだけで、手元では開発サーバーも動いており、どちらを見ているのか見分けが付かないためです。Wi-Fi を切ってモバイル回線にしたスマートフォンで同じ URL を開き、自分の家の回線を1つも通らない端末で画面が出れば確実です。

**Q2. `prisma db push` を本番の DB に対して実行してよいのは、どんなときですか。**

A. Step 4 で新しく作った、この教材用の本番 DB に対して1回だけです。実務では `prisma migrate deploy` を使います。この教材は migration の履歴を作る手順を扱っていないため、中身が空の DB に限って `db push` で表を作ります。すでにデータが入っている DB に、この手順をそのまま持ち込むことはできません。
