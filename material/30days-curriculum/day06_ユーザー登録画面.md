...
- ブラウザで開くのは、このあと3つ目のブロックまで書き終えてから
...
1行ずつが「これはブラウザで止める」という宣言です。`name` の `.min(1)` を外すと、名前が空のままでも送信ボタンが通り、断られると分かっているリクエストがサーバーへ飛びます。`email` の `.email()` を外すと `abc` のような文字列でも送信され、間違いを知るのは通信の返事が返ってきたあとになります。`password` に並ぶ4本の `.regex()` は、大文字・小文字・数字・記号をそれぞれ1文字以上求めます。同じ4本は Day 07 で作るサーバー側の `auth.ts` にも書いてあり、登録を最終的に許すかどうかを決めるのはそちらです。ブラウザ側のこのスキーマが受け持つのは、打ち終わったその場で間違いを知らせることと、通らない通信を減らすことの2つです。最後の `.refine()` は、項目ごとのチェックが済んだあとに、2つの欄をまたいで見る確認です。項目側でエラーが出ていても、続けられる種類のエラーなら `.refine()` も走ります。短すぎるパスワードで確認欄も違っていれば、両方のエラーが同時に出ます。
...
送信を試すのは Step 9 からです。フォームを送るボタンは、まだ書いていません。

スクリーンショット: 下の画像は Step 9 まで書き終えた完成後の画面です。赤枠の中が、この Step で足した名前とメールの2欄です。いまの自分の画面には、その下のパスワードとパスワード(確認)の欄がまだ出ていません。パスワード欄は Step 5、パスワード(確認)欄は Step 6 で足します。
...
8文字未満で弾かれることは、送信ボタンがそろう Step 9 で確かめます。
...
`path` を書かなかった場合、エラーはフォーム全体に付きます。`errors.confirmPassword` は空のままなので、どの入力欄の下にも赤い文字が出ません。送信ボタンがそろった Step 9 以降で見えるのは「登録ボタンを押しても画面が何も変わらない」状態だけになり、どこを直せばよいのか手掛かりがありません。エラーは出すだけでなく、出す場所まで指定して初めて役に立ちます。

**確認ポイント**:
- パスワード確認欄が表示されている
- `npm run dev` で型エラーが出ていない

2つのパスワードが食い違うときの動きは、送信ボタンがそろう Step 9 で確かめます。
...
**確認ポイント**:
- 「登録」ボタンが表示されている
- 空のまま押すと、名前欄とメール欄の下にエラーメッセージが出る
- パスワードを7文字で押すと、文字数のエラーが出る
- 2つのパスワードを食い違わせて押すと「パスワードが一致しません」がパスワード(確認)欄の下に出る

Step 4 から Step 6 で書いた検証は、ここで初めて動かせます。送るきっかけになるボタンが、この Step でそろったためです。
...
`password123` が、今日 Step 2 で決めた「大文字・小文字・数字・記号をそれぞれ1つ以上」という規則を満たしていないことに気づいた人もいるはずです。矛盾ではありません。あの規則は登録画面が新しいパスワードを受け取るときの関門で、ログイン画面は通しません。ログイン側は入力が空でないかだけを見て、あとは保存済みのものと一致するかを照合します。規則を厳しくしたあとで既存の利用者がログインできなくなる事態を避けるためです。`admin@example.com` は登録画面を通らずに初期データとして直接作られたアカウントなので、登録時の規則も通っていません。
...
> **完成形の参考コード**: 完成版には `src/app/register/page.tsx` があります。ただし今日書いたコードと1文字まで同じではありません。違うのは次の4か所です。1つ目は、完成版のパスワード条件が `.min(8)` だけで、大文字・小文字・数字・記号の `.regex()` が書かれていない点です。2つ目は、登録に成功したあとの移動先が `/dashboard` ではなく `/login` で、`router.refresh()` も呼ばない点です。3つ目は、エラーの入れ物が `useState<string[]>([])` になっていて、サーバーから返った複数のメッセージを並べて出せる形になっている点です。4つ目は、完成版の入力欄に `aria-required` や `aria-invalid` といった読み上げ用の指定が付いている点です。読み上げソフトへの対応は Day 10 でまとめて扱います。5つ目は、完成版の背景に `blur-3xl` のぼかし装飾があり、色を `bg-primary` などのテーマ名で指定していて、必須の入力欄に `text-destructive` の赤い印が付いている点です。今日は色を `from-blue` `to-indigo` と直に書いています。見比べるときは、この5か所は違って当たり前だと思って読んでください。（販売用 ZIP に完成版の `src/` は入っていません。ここに挙げた違いは、完成版がどう書かれているかの説明として読んでください）。
...
**Q1. `.refine((data) => data.password === data.confirmPassword, { path: ['confirmPassword'] })` は、何をしている検査ですか。**

A. 各項目の個別チェックが済んだあとに動く、最後の見比べです。パスワード欄と確認欄の2つを突き合わせ、違っていればエラーにします。個別チェックは1つの欄しか見ないので、2つの欄をまたぐ比較はここでしか書けません。`path` は、そのエラーをどの欄の下に出すかの指定です。

**Q2. その `path: ['confirmPassword']` を書き忘れると、画面はどうなりますか。**

A. エラーがフォーム全体に付き、`errors.confirmPassword` は空のままになります。どの入力欄の下にも赤い文字が出ません。利用者から見えるのは「登録ボタンを押しても画面が変わらない」という状態だけです。どこを直せばよいかの手掛かりが、画面に1つも残りません。

**Q3. `mutate` に `confirmPassword` を渡していないのは、なぜですか。**

A. 確認欄は、打ち間違いをその場で見つけるための画面側だけの欄だからです。サーバーは保存するパスワードを1つ受け取れば足ります。同じ値をもう1つ送っても、サーバー側ですることはありません。送る項目が少ないほど、途中で漏れる範囲も狭くなります。
