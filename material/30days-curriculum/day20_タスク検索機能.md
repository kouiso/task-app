...
ここで取り込む部品は、すべて Day 09 から Day 19 までに使ってきたものです。`TaskCard` は Day 13 のタスク一覧で、`DeleteConfirmDialog` は Day 11 の削除確認で初めて呼び出した、用意済みの共通部品です。どちらも中身を自分で書いたことはありません。検索画面でも表示用の部品を新しく作らず、既にあるカードとダイアログを並べ替えて使います。見た目がタスク一覧とそろうので、読者にとっても「検索したあとの操作は今まで通り」になります。
