...
ステータスが変わり、完了日時（`completedAt`）が明示されていないときだけ、完了日時を自動で決めます。`DONE`（完了）へ変えたら今の時刻を入れ、それ以外へ戻したら `null` にします。同じステータスのままタイトルを直しても元の完了日時は変わりません。
...
前半は 0-3 と同じで、送られてきた項目だけを詰めます。`dueDate` と `completedAt` は値があれば `new Date(...)` で日付に変換し、空なら `null` にします。最後の2行はプロジェクトの移動が起きるかどうかを判定しています。`isProjectChanging` は「新しい `projectId` が送られていてしかも今のプロジェクトと違う」ときだけ真になります。`targetProjectId` は移動するなら移動先、しないなら今のプロジェクトを指します。
...
完了日時を自動で変えるのはステータスが変わり、画面から完了日時が送られていないときだけです。同じ `DONE` のままタイトルを直した場合は元の完了日時を保ちます。完了日時を明示した場合はその値を後のブロックで設定します。
