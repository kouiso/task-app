...
スクリーンショット: 今日を終えたコメント欄です。自分のコメントの編集ボタンを押すと、その行だけがテキストエリアに変わります。
...
関門は3つあり、守っているものがそれぞれ違います。2つ目だけだと、同じプロジェクトの同僚が他人のコメントを書き換えられます。3つ目だけだと、そもそも参加していない人にコメントの中身が返ります。
...
Day 18 の投稿フォームには `defaultValues` を書きましたが、こちらには書きません。編集は必ず「編集ボタンを押す」から始まり、そのハンドラーが `setValue` で今の本文を入れてからテキストエリアを出すためです。値が `undefined` のまま `watch` を呼ぶ場面がありません。投稿フォームは押す前から画面に出ているので、そちらは初期値が要ります。
...
外側の `justify-between` は、子要素を左端と右端へ振り分けます。子が「名前」「日時」の2つなら名前が左、日時が右で収まります。ここへボタンを2つ足すと子が4つになり、日時とボタンが離れて散ります。日時とボタンを1つの箱に入れれば子は2つに戻り、右端の箱の中でボタンが日時の隣に並びます。
...
スクリーンショット: 下の画像は Step 6 まで書き終えた完成後の画面です。赤枠の中が、この Step で足したペンとゴミ箱のボタンです。初期データの「デザインモックアップ作成」には自分のコメントが1件入っているので、その行にだけ2つが並びます。
...
編集モードの画面を目で見るのは Step 6 のあとになります。削除の処理を書くまで、この画面はエラーで止まるためです。
...
Step 4 で書いた編集モードは、ここまでで呼び出しがつながります。ただし画面で試せるのは Step 6 のあとです。この時点では `handleDeleteComment` がまだ無く、JSX がそれを参照しているためエラーで止まります。下の画像は Step 6 まで書き終えた画面で、タスク詳細を開いて自分のコメントの編集ボタンを押すと、その行だけテキストエリアに変わります。キャンセルを押せば元の表示へ戻ります。
...
タスク削除でも使った再利用コンポーネントです。配布済みの部品なので、新しく作る必要はありません。
...
スクリーンショット: ゴミ箱のボタンを押すと、この確認ダイアログが出ます。見出しは `title` に渡した文言です。
...
初期データでは、「デザインモックアップ作成」に自分（`admin@example.com`）のコメントが1件、「データベース設計」に田中太郎のコメントが1件入っています。1つのタスクに自分と他人のコメントが並んだ状態は初期データには無いので、まず「デザインモックアップ作成」を開いて、自分の行にアイコンが2つ並ぶことを確かめてください。
...
**Q1. `findCommentAndAssertOwnership` は、コメントを1件引くときに何を一緒に取ってきていますか。3つ挙げてください。**

A. コメントの作者である `userId`、そのコメントが属するタスクとプロジェクト、そしてそのプロジェクトのメンバーのうちログイン本人の行だけ、の3つです。3回に分けて問い合わせると、その合間にメンバーから外れるといった食い違いが起きます。1回でまとめて取れば、判定に使う材料がすべて同じ時点のものになります。

**Q2. `comment.userId !== userId` のチェックを消して、メンバー確認だけを残すと何ができてしまいますか。**

A. 同じプロジェクトのメンバーであれば、他人のコメントを書き換えたり消したりできてしまいます。逆に作者確認だけを残してメンバー確認を消すと、プロジェクトから外れた人が過去の自分のコメントを操作できます。2つは別のことを守っているので、片方だけでは足りません。

**Q3. 編集中かどうかを `boolean` ではなく `editingCommentId: string | null` で持つのは、なぜですか。**

A. 真偽値1つだと、編集ボタンを押した瞬間に並んでいるコメントが全部テキストエリアへ変わるためです。どのコメントを編集中かを id で覚えておけば、`editingCommentId === comment.id` の比較で1件だけを切り替えられます。編集をやめるときは `null` に戻します。
