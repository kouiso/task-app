...
📸 スクリーンショット: ユーザー管理ページ全体の表示を確認してください。

![ユーザー管理ページ全体の表示を確認してください。](./screenshots/user-list.png)
## 🤔 なぜこれを作るのか？
...
✅ **確認ポイント**:
- `date-fns` の `format` と `ja` をインポートしている
- `Eye`, `Pencil` アイコンをインポートしている
...
- `USER_ROLE` 定数をインポートしている
- `PageLoadingSpinner` をインポートしている
...
> 💡 `PageLoadingSpinner` は `AppLayout` を
> 内包しているため、サイドバーやヘッダーが
> 自動的に表示されます。
...
📸 スクリーンショット: ユーザー管理ページ（管理者がアクセスした画面）の表示を確認してください。

![ユーザー管理ページ（管理者がアクセスした画面）の表示を確認し](./screenshots/user-list.png)
#### 権限チェックの判定ロジック
...
> 💡 `USER_ROLE.ADMIN` は
> `@/lib/constant/roles` で定義された
> 定数です。文字列リテラル `'ADMIN'` を
> 直接書かず、定数を使うことで
> タイプミスを防げます。
> `currentUser?.role !== USER_ROLE.ADMIN` は
...
✅ **確認ポイント**:
- 管理者は Shield アイコン付き、ユーザーは User アイコン付きで表示される

```typescript
// filepath: src/app/user/page.tsx
// ステータスバッジ（専用コンポーネントを使用）
<TableCell>
  <ActiveStatusBadge
...
📸 スクリーンショット: アバターとバッジの表示を確認してください。

![アバターとバッジの表示を確認してください。](./screenshots/user-list.png)
#### バッジコンポーネントの仕様

`UserRoleBadge` と `ActiveStatusBadge` は
`@/component/ui/user-badges` に定義された
専用コンポーネントです。
...
📸 スクリーンショット: 完成したユーザー管理ページの表示を確認してください。

![完成したユーザー管理ページの表示を確認してください。](./screenshots/user-list.png)
...
| ActiveStatusBadge | アクティブ/無効の状態バッジコンポーネント |
