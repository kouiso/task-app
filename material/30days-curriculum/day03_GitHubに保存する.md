...
`fatal` が出たら、その行から無いファイル名だけを外して同じコマンドをもう一度実行してください。そのあと `git status --short` で、変更したファイルの行頭に `M` や `A` が付いた（ステージングされた）ことを確認します。`README.md` に加え、上で挙げた実在するファイルがすべてステージングされていることを確認します。アプリのコードや設定が抜けると、Day 04 のビルドに失敗します。
...
- 秘密の値を入れる `.env` や `.env.local` が出ていない。練習用の値だけを入れた `.env.example` の行はあってよい
...
コミットには、このメールアドレスも記録されます。公開リポジトリでは他の人も読めるため、公開したくないアドレスは使いません。GitHub の [メール設定](https://github.com/settings/emails)で `Keep my email addresses private` を有効にし、表示される `noreply` アドレスを上のコマンドへ入れてください。
GitHub に登録した通常のアドレスを公開してよい場合は、そのアドレスも使えます。どちらも GitHub がコミットと自分のアカウントを結び付けるための値です。
...
`.env.example` は、練習用の値を入れた見本です。本番のパスワードなどへ書き換えていないことを確認してから追跡します。
