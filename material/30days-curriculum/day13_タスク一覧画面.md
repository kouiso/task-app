...
| ステップ | 作業内容 | 所要時間 |
|---------|---------|---------|
| Step 0 | タスク一覧 API（getAll）を自分で書く | 12分 |
| Step 1 | ページの土台を作る | 5分 |
...
**ゴール**: タスク一覧を返す `getAll` を自分で書き、`root.ts` に登録して、画面から `api.task.getAll` を呼べる状態にします。
...
今日はこの3部品のうち、一覧取得の `getAll` だけを書きます。`create` や `update` は、それを実際に使う Day 14 以降で1つずつ足していきます。
...
`orderBy` は `position`（並べ替え用の番号）の昇順で、同じなら作成日の新しい順にします。`take` と `skip` は取得件数と開始位置の指定です。最後の `});` で findMany を閉じ、`}),` で `getAll`、`});` で `taskRouter` 全体を閉じます。

**確認ポイント**:
- `src/server/api/routers/task.ts` に `getAll` を書き、`}),` と `});` まで閉じた
- `getUserProjectIds` を使って自分のプロジェクトのタスクだけに絞っている
- `npm run dev` で型エラーが出ていない（この API を画面から呼ぶのは Step 2 以降なので、今は起動時にエラーが出なければよい）

#### 0-7. root.ts に task ルーターを登録する

`getAll` を書いただけでは、まだ画面から呼べません。作った `taskRouter` を `root.ts` に登録して、初めて `api.task.getAll` という呼び名が生まれます。Day 09 で `project` を登録したのと同じ形です。
...
`appRouter` に `task: taskRouter` を足したことで、フロント側の `api.task.getAll` がこの手続きにつながります。今の `root.ts` には auth・project・task の3つが並びます。`comment` や `search` などは、それを使う Day で1つずつ足していきます。
