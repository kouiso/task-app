...
![4枚の統計カードとプロジェクト統計テーブルが並んだレポートページ。下に写っている円グラフは Day 22 の成果物](./screenshots/report.png)

画像の下半分に写っている円グラフは Day 22 で作ります。今日の終わりには、カードとテーブルまでが出ていれば正しい状態です。
...
カードとテーブルはまだ出ません。下の画像は完成版の姿で、カードとテーブルに加えて
Day 22 の円グラフまで写っています。いまは上の2行だけが見えていれば正しい状態です。
...
![4枚の統計カードが横に並んだレポートページ](./screenshots/report.png)

この画像は完成後の姿なので、カードの下に円グラフも写っています。グラフを作るのは Day 22 なので、今日の画面ではカードの下は空のままです。カードが4枚並んでいれば正しい状態です。
...
レスポンシブ確認: ブラウザの幅を狭めていくと、カードの並びが4列から2列、1列へと変わります。下の画像は PC 幅のときの姿で、4列に並んだ状態です。モバイル幅の姿は、ブラウザの幅を実際に縮めて確かめてください。

![PC 幅のレポートページ。統計カードが横に4列並んでいる](./screenshots/report.png)
