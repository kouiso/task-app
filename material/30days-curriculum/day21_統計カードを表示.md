...
数字は初期データのものです。読者の手元と件数が違っても、実装の誤りではありません。
...
`api.report.getOverview` は、完成版のコードで
...
骨組み確認: 下の画像は Step 8 まで書き終えた完成後の画面です。赤枠の中が、この Step で出るところです。いまの自分の画面には、その下のカードとテーブルがまだ出ていません。カードは Step 7、テーブルは Step 8 で足します。
...
カード確認: 下の画像は Step 8 まで書き終えた完成後の画面です。赤枠の中が、この Step で足した4枚のカードです。いまの自分の画面には、その下のプロジェクト統計テーブルがまだ出ていません。テーブルは Step 8 で足します。
...
`PORT=3001` を付けているのは、他に動かしている開発サーバーと番号を取り合わないためです。起動したら、まずカードの数字がシードデータと合っているかを確かめます。総タスク数が初期データの5件より少なくても正常です。ログイン中のユーザーが参加しているプロジェクトのタスクだけを数えるためです。管理者アカウントは2つあるプロジェクトのうち1つにしか参加していないので、3件と表示されます。逆に多いときは、Step 0-3 の `activeTasksFilter` をどこかの `count` で使い忘れています。4枚とも 0 で、テーブルも空のときは、参加しているプロジェクトをアーカイブしたままです。`/project` でアーカイブ表示を ON にして解除すると数字が戻ります。
...
レスポンシブ確認: ブラウザの幅を狭めていくと、カードの並びが4列から2列、1列へと変わります。下の画像はブラウザの幅を 900px まで縮めたときの姿で、`lg:grid-cols-4` が外れて `sm:grid-cols-2` が効いています。
...
A. `NOT: { status: TASK_STATUS.CANCELLED }`、つまり中止したタスクを除く条件を足したものです。`projectScope` の側は「自分が参加中で、しかもアーカイブされていないプロジェクトのタスク」までを決めています。2段に分けてあるので、中止を数えたい集計では `projectScope` のほうを使えます。

**Q2. 完了率の分母を `activeTasksFilter` の件数から `projectScope` の件数に変えると、数字はどう動きますか。**
