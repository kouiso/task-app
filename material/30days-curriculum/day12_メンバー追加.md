...
`removeMember` の下に追加します。`ProjectDetailView` の中には、メンバーのロールを変えるセレクトボックスがあります。これは Step 6 で書く `handleUpdateMemberRole` から呼ばれ、その先でこの `updateMemberRole` procedure を叩きます。先にサーバー側の手続きを用意して、Step 6 のクライアント側とつなげます。
...
> メンバー一覧の表示は `ProjectDetailView` の内部で行われます。`page.tsx` はデータ取得・権限計算・イベントハンドラーの定義を担当し、UIの詳細は独立コンポーネントに任せます。`handleUpdateMemberRole` は Step 5 で本実装します。
...
- `isProjectMemberRole` 型ガード関数と `hasPermission` をインポートしている
...
続いて、Step 2 の `ProjectDetailView` に渡した `handleUpdateMemberRole` を実装します。`removeMemberMutation` の並びにロール変更の mutation とハンドラーを追加してください。
...
**確認ポイント**:
- `ProjectDetailView` の `onUpdateMemberRole` に渡す関数が実装できた
- 成功時に `getById` を再取得してロール表示を更新している

ロール変更のセレクトボックスは `ProjectDetailView` の内部にあります。`page.tsx` は「どのメンバーをどのロールにするか」を受け取って mutation を呼ぶだけで、UI そのものは持ちません。Step 0-5 で書いた `updateMemberRole` procedure が、この mutation の呼び先です。
