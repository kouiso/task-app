...
**型**: `ProjectDetail | null | undefined`
...
**型**: `(userId: string) => void`

**役割**: メンバーの削除処理を実行します。

**`onUpdateMemberRole`**

**型**: `(userId: string, role: ProjectMemberRole) => void`

**役割**: メンバーのロール変更を実行します。

**`onArchive`**

**型**: `(projectId: string, isArchived: boolean) => void`
...
**役割**: メンバー管理ボタンを表示するかどうかを表します。

**`canArchive`**

**型**: `boolean`

**役割**: アーカイブボタンを表示するかどうかを表します。
...
`onSuccess` で `getById.invalidate()` を呼んでいるか確認してください。

#### 「この操作を実行する権限がありません」

**原因**

MEMBER/VIEWER で管理操作を試行したためです。

**解決方法**

OWNER/ADMIN アカウントでログインしてください。

#### `@prisma/client` からインポートエラー
...
`@/lib/constant/roles` からインポートしてください。
