...
![プロジェクト削除の確認ダイアログ。キャンセルと削除のボタンが並ぶ](./screenshots/project-delete-confirm.png)

画像はプロジェクトを消すときのもので、背景も一覧ページです。
いま作った側は、プロジェクト詳細ページのメンバーカードから開きます。
`title` に `このメンバーを削除しますか？` を渡しているので、見出しもそちらに変わります。
確かめたいのは見出しの文字ではなく、ボタンが2つ並んで押せる状態になっているかです。
