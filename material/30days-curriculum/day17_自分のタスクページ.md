...
> ⚠️ **Reactの hooks ルール**: `useQuery` や `useUtils` などの hooks はコンポーネントのトップレベルに配置し、`if` 文や `return` の後に置いてはいけません。hooks の呼び出し順序が変わるとエラーになるため、early return（`if (isLoading) return ...`）は必ず全 hooks 定義の後に書きます。
...
🎯 **ゴール**: タスクを期限で4つのグループに分類します。完成版 source と同じく `dateOnlyFromValue()` と `localDateOnly()` を使い、日付だけを比較します。
...
```typescript
// filepath: src/app/my-task/page.tsx
// tasks が undefined の場合に備えて先に確定させる
const allTasks = tasks ?? [];
// 要素1件分の型を取り出す（undefined を除去してから）
...
| `startOfDay` どうしの比較 | 実装はできるが、教材では `isSameDay` の方が意図が伝わりやすい | △ |
| `isSameDay(dueDate, now)` | 時刻を無視して日付だけを正確に比較できる | ✅ |

> 💡 完成版 source では、`dueDate` を
...
> ⚠️ `const` は同一スコープで再宣言できません。Step 9・10 では、上の2行を**削除してから**本実装を書いてください。
