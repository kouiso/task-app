...
`enabled` の条件が `false` のままだからです。`currentUser` の取得が失敗した場合や、`enabled: false` と書いた場合は `task.getAll` を呼びません。`enabled` を省くと通信自体はすぐ始まり、`assigneeId` が `undefined` の間は全タスクが対象になります。

**解決方法**

DevTools の Network タブで `getCurrentUser` が成功しているか確認し、`task.getAll.useQuery` の第2引数を `{ enabled: !!currentUser }` にしてください。ユーザー情報の取得後に `task.getAll` が始まります。通信が成功して0件の場合は、担当者がログイン中の自分か、ステータスとプロジェクトの絞り込みが残っていないかも確認します。
...
`isCurrentUserLoading` または `isLoading` の判定が抜けているためです。

**解決方法**

`isCurrentUserLoading || isLoading` の両方を確認してください。
