...
`onCheckedChange`ハンドラーで`Set`を正しく更新していないためです。`Checkbox`は`onChange`ではなく`onCheckedChange`を受け取ります。

**解決方法**

`new Set(prev)`でコピーを作ってから`add`/`delete`してください。直接mutateしないでください。

#### `indeterminate`状態が表示されない

**原因**

`checked`propに`true`/`false`しか渡していないためです。

**解決方法**

shadcn/uiの`Checkbox`は`checked`に文字列`'indeterminate'`を渡すと部分選択状態になります。

#### 一括操作後にチェックが残る

**原因**

操作成功後に`selectedTasks`をクリアしていないためです。

**解決方法**

`onSuccess`内で`setSelectedTasks(new Set())`を呼んでください。

#### `updateMany`で型エラーが出る

**原因**

`status`に文字列をそのまま渡しているためです。

**解決方法**

`isTaskStatus`型ガードで検証してから渡してください。

#### 全選択の表示が選択件数と合わない

**原因**

比較対象に一覧全体の `tasks` を使っているためです。

**解決方法**

`selectedTaskList.length` と操作可能な `selectableTasks.length` を比較してください。

### 詰まりやすいポイントまとめ

#### `selectedIds` という変数名でエラーになる

**原因**

実際のコードは `selectedTasks` を使っているためです。

**解決方法**

変数名を `selectedTasks` / `setSelectedTasks` に統一してください。

#### 型エラー: `string` is not assignable to `TaskStatus`

**原因**

`isTaskStatus` 型ガードがないためです。

**解決方法**

`if (isTaskStatus(value))` で囲んでから呼んでください。

#### 削除が確認なしで即実行される

**原因**

`setBulkDeleteDialogOpen(true)` を呼んでいないためです。

**解決方法**

`handleBulkDelete` でダイアログを開く流れに修正してください。
...
`onSuccess` の中で `utils.task.getAll.invalidate()` を追加してください。

#### チェックが入ったまま残る

**原因**

`setSelectedTasks(new Set())` を呼んでいないためです。

**解決方法**

`onSuccess` で空の Set にリセットしてください。

#### 操作できないタスクまで全選択される

**原因**

`handleSelectAll` が `tasks` からIDを集めているためです。

**解決方法**

`selectableTasks.map` で操作可能なタスクのIDを集めてください。
