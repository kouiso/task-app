...
権限の確認が入口と書き込み時の2回入っているのは書き忘れではありません。入口の `assertMemberPermission` は権限のないタスクが混ざっていたら1件も書き込まずに止めるための門です。書き込み時の `where` はその門を通ったあとにロールが変わった場合を拾います。`completedAt` を `status` と一緒に入れているのはDay 23 の週次レポートが完了件数をこの列から数えるためです。
...
`DONE` 以外へ動かしたときに `completedAt` を `null` へ戻しているのが見落としやすい分岐です。完了から進行中へ差し戻したタスクに完了日時が残ったままだとDay 23 の週次レポートが未完了のタスクを完了件数に数え続けます。`data` に型を付けてあるのはあとから列を足し引きする書き方をしているためで、綴りを間違えた列名がその場で赤くなります。
