...
図の左側の `/report` は Day 21・22 で作ったページです。今日はそこへプロジェクト統計テーブルを1枚足し、右へ渡るリンクを付けます。右側の `/report/weekly` は今日はじめて作るページで、サマリーカード3枚とグラフ3枚が並びます。
...
`isLoading` のうちに `return` で打ち切るのも欠かせません。ここを飛ばすと、まだ `undefined` の `reportData` を後ろのグラフが読みにいき、画面がエラーで止まります。Day 09 のプロジェクト一覧でも、`projectsLoading` が真の間はスピナーを返して先へ進ませませんでした。
...
`{reportData?.totalCompleted ?? 0}` の `?? 0` は保険です。`isLoading` を抜けたあとでも、通信が失敗すれば `reportData` は `undefined` のまま残ります。この保険が無いと、そのときカードの数字の場所には何も描かれず、見出しだけが浮いた空のカードになります。0 と出ていれば「まだ完了が無い」と読めます。空欄のままでは、その判断すらできません。
...
そのため、積み上げ棒の「進行中」と「レビュー中」は、ふだん 0 のまま伸びません。Day 15 で書いた更新処理が、ステータスを完了へ変えた瞬間に完了日時を入れ、完了から戻すと `null` へ戻すからです。棒が伸びるのは、完了日時が残ったまま別のステータスへ動かされた行だけになります。グラフが平らでも壊れてはいないので、驚かなくて大丈夫です。
...
先頭が `return await prisma.task.findMany({` になっているのが Before との違いです。Before はいったん `const tasks` で受け取り、そのあと加工していました。After は取得結果をそのまま返すので、途中に追加の問い合わせを差し込む場所そのものがありません。`where` の中身は Before と変えておらず、直したのは次のコードブロックに続く `select` の書き方だけです。
