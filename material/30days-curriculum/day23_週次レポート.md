...
スクリーンショット: 今日の出発点になるレポートページです。Day 21 のカードと Day 22 の円グラフが並んでいます。

![レポートページ。統計カード4枚、円グラフ2枚、プロジェクト統計テーブルが縦に並んでいる](./screenshots/day23/report.png)
...
左は入れ子、右は入れ子のないひと並びです。Recharts は右の形しか読めないので、同じ材料から2種類のひと並びを作ります。グラフの棒が出ないときは、`dataKey` に書いた名前が右側の列名と合っているかを見てください。
