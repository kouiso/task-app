...
スクリーンショット: 今日の出発点になるレポートページです。Day 21 のカードと Day 22 の円グラフが並んでいます。Day 22 を終えた時点の画面なので、今日の成果である週次レポートへのリンクはまだありません。
...
スクリーンショット: 読み込んでいる間は、スピナーだけが出ます。
...
レポート見出しの右側へリンクを置きます。並べるには囲みが要ります。まず見出しと説明文を `<div>` で1つにまとめてください。その外側を `flex flex-col gap-4 items-start sm:flex-row sm:items-center sm:justify-between` を付けた `<div>` で包みます。この囲みが無いと、リンクは見出しの下へ落ちます。次のコードは、その囲みの中、見出しをまとめた `<div>` の後ろへ貼ります。`Link` は Next.js が用意したページ移動用の部品で、`<a>` タグと違って移動先の中身だけを差し替えます。だからサイドバーやヘッダーは表示されたまま残り、画面が一度白くなってから描き直される動きになりません。`ArrowRight` は右向き矢印のアイコンで、押した先へ進むことを文字より速く伝えます。
...
左は入れ子、右は入れ子のないひと並びです。Recharts は配列の各オブジェクトから、`dataKey` に指定したキーの値を読みます。ここではキーを直接指定しやすい形へ2種類の配列を作ります。グラフの棒が出ないときは、`dataKey` に書いた名前が右側の列名と合っているかを見てください。
...
A. 1つ目は `completedAt` が `null` の行で、まだ終わっていないタスクです。2つ目は `assigneeId` が対象のユーザーと違う行で、他の人が片づけた仕事にあたります。3つ目は `startDate` より前に完了した行で、指定した週数の外になります。週次レポートは「この人がこの期間に終わらせた量」を出す画面なので、残っているタスクの件数は扱いません。
...
A. 境界ちょうどに完了したタスクが、前の週と次の週の両方で数えられるのを防ぐためです。「開始以上・終了未満」でそろえておけば、どのタスクもどれか1つの週にだけ入ります。
