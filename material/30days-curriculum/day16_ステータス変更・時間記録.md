...
1つ先に伝えておくことがあります。配布コードには Day 13 の時点で、
...
Day 05 のログイン画面では、入力値を `useState` だけで
持ちました。今回は入力値に加えて、項目ごとのエラー文も
抱えることになります。手で書くと、送信のたびに検証を
...
`Dialog` の `open` には親から届いた `open` を渡し、
`onOpenChange` には `handleClose` を渡しています。背景を
クリックしたときも、Esc キーを押したときも、この
...
`useCallback` で包み、依存に `utils.task.getAll` を書いてあるのは、この関数の中身がそれだけに頼っているためです。包まずに書くと、再描画のたびに別の関数が生まれます。受け取る `TaskCard` からは、中身が同じでも毎回違う props が届いたように見えます。
