...
<!-- TODO(screenshot): 上の task-timer.png は開きかけ（半透明）の瞬間を捉えたキャプチャです。ダイアログが完全に開いた状態で撮り直します。なお従来ここに貼られていた task-detail-dialog.png はタスク詳細ダイアログで、確認ポイント（時間と分の入力欄）と一致しないため差し替えました -->
