...
スクリーンショット: 今日を始める前のタスク詳細ダイアログです。ステータスは表示されるだけで、ここから変える操作はまだありません。今日はこれを変えられるようにして、作業時間も記録できるようにします。
...
スクリーンショット: 下の画像は Step 3 まで書き終えて、カードの「時間記録」ボタンから開いた状態です。いまの自分の画面には、まだこれを開く入口がありません。入口は Step 3 で足します。
...
行き帰りで通る形は「分の合計」1種類だけです。`1時間30分` と `13h 30m` は、画面のすぐ手前でだけ現れる形です。データベースを覗いても `13h 30m` という文字は見つかりません。
...
Step 2 で書いた検証は、開く入口がそろったここで初めて動かせます。
...
初期データのタスクには作業時間があらかじめ入っているので、`12h 0m` や `20h 0m` のように0でない数字が並びます。3枚目の `1h 15m` は、いまの手順で1時間15分を記録した直後の状態です。記録した時間は、もともと入っていた合計へ足されます。
