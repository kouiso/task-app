...
エラーがサーバー側とブラウザ側のどちらで起きたかは、画面ではなく Console で見分けます。
...
今回の `throw` なら「テストエラー: これは練習です」がそこに並びます。これがブラウザ側で投げたエラーの見え方です。
サーバー側で投げたエラーは、本番ビルドだと文言が伏せられ、`error.digest`（エラー1件ごとに振られる短い符号）だけが渡ります。
Console に文言が無く符号だけが出ていればサーバー側で、本当の文言は `npm run dev` を動かしているターミナルのログに出ています。
