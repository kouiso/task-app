...
`'use client'` を先頭に書いているのは、Error Boundary がブラウザ側でしか動かないためです。
サーバー側で組み立てた HTML には「エラーを捕まえて描き直す」動きを埋め込めません。
...
拾えないものもあります。
ボタンの `onClick` の中で投げたエラー、`fetch` の結果を待ってから投げたエラーは、描画が終わったあとに起きるので `error.tsx` は反応しません。
この2つは Console に赤い行が出るだけで、画面は何ごともなかったように残ります。
...
`error.tsx` を置いていなければ、ブラウザには何も描かれず真っ白なページだけが残ります。
何が起きたのかは画面から分からず、Console を開いてやっと赤い行が見つかります。
初心者にとっていちばん心細いのがこの状態で、Step 1 で置いた `error.tsx` はこれを防ぐために書きました。

エラーがサーバー側とブラウザ側のどちらで起きたかも、出てくる画面から見分けられます。
ブラウザ側で投げたエラーは `error.message` に文言がそのまま入るので、画面に出せます。
サーバー側で投げたエラーは、本番ビルドだと文言が伏せられ、`error.digest`（エラー1件ごとに振られる短い符号）だけが渡ります。
中身をそのままユーザーへ見せないための仕組みで、本当の文言はサーバーのログに出ています。
画面に「テストエラー: これは練習です」と出ればブラウザ側、記号の列しか出なければサーバー側だと判断できます。
...
見分け方はこうなります。
Network タブに赤い行（400 番台や 500 番台）が並んでいればサーバー側の問題、緑のまま同じ行が積み上がっていればブラウザ側の問題です。
...
30日ぶんのコードが積み上がった今、毎回プロジェクト全体にかけると出力が長くなり、いま足した2行の指摘が埋もれます。
...
Step 5 では1ファイルに絞りましたが、ここではプロジェクト全体を見ます。
`npm run lint` の中身は `biome check .` で、末尾の `.` が「このフォルダから下すべて」という意味です。
