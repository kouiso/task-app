...
今日は、このアプリを Vercel を使ってインターネット上に公開します。公開して URL が発行されると、自分のパソコン以外からもアプリを開けるようになります。スマホやほかの人のブラウザからもアクセスでき、いま作っているものを実際に見せられる状態になります。
...
Day 03 で GitHub へ保存した自分の `task-app` を Vercel につなぎ、実際の URL で公開できるようになります。デプロイボタンを押すだけで終わらせず、次のことができるようになります。
...
新語の表で「環境変数＝店の裏の金庫」と例えました。今日はその金庫がどこにあるか、つまりこの Environment Variables の欄の場所を見ておくだけで十分です。中に何を入れるかは、鍵が必要になる Day 30 で扱います。
...
【スクリーンショット】ローカル本番モードで開いたダッシュボード（公開 URL 確認の代わりの例）
![ローカル本番モードで開いたダッシュボード画面](./screenshots/day04/public-url.png)

Vercel の Production URL がまだ使えない場合は、`npm run build` でビルドしてから `npm start` で立ち上げた、ローカルの本番モードを開いて確認してよいです。`npm run dev` が開発中の確認用なのに対して、`npm start` はビルド済みのものを本番と同じ状態で動かすコマンドです。公開 URL とほぼ同じ表示になるので、代わりの確認に使えます。

上のキャプチャも、この方法で `/dashboard` を開いた例です。だからキャプションも「公開 URL の画面」ではなく「ローカル本番モードの画面」と書いてあります。
...
Day 03 では `.gitignore` と `.env.example` の役割を確認しました。本物の値はリポジトリに送らず、見本だけを共有するという線引きです。API キーやデータベース接続情報のようなシークレットを、GitHub には含めません。こうした値は公開せず、Vercel の Environment Variables に直接登録します。今日は環境変数を追加しません。実際に設定するのは、本番公開を仕上げる Day 30 です。なお `NEXT_PUBLIC_` で始まる環境変数はブラウザにも渡ります。秘密の値には使いません。
