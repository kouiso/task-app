...
このコマンドは、Day 01 から使ってきた `npm run dev` とは別の道を通ります。`dev` は書き換えた結果をすぐ画面へ反映するためのコマンドです。型が合わないコードも JavaScript に変換して実行するため、ページを開けても型が正しい証拠にはなりません。`build` は本番用の完成品を1回だけ作る処理で、途中で全ファイルの型をまとめて確認します。だから、まだ開いていないページに残った型のずれもここで止まります。そして Vercel が公開のときに走らせるのも、この同じ `build` です。手元で通ることを先に確かめておけば、Vercel で同じ場所につまずく可能性をあらかじめ減らせます。
...
このあと Deploy を押すと、画面の名前やメッセージもインターネットから読めるようになります。`src/app/dashboard/page.tsx` を開き、実名や勤務先の情報などがあれば、公開してよいニックネームや練習用の文へ置き換えます。変更した場合は、公開前に次を実行してください。

```bash
npm run build
git add src/app/dashboard/page.tsx
git diff --cached
git commit -m "docs: use public dashboard sample text"
git push
```

`git diff --cached` は、今回送る変更の内容を表示します。公開してよい文だけになっていることを読んでから、コミットと push を進めます。何も変更していなければ、この操作は不要です。
...
flowchart TB
    ...
    EX[".env.example<br/>設定名と練習用の値の見本<br/>GitHub へ送る"]
...
同じ名前の設定値が3か所に分かれて置かれます。本物の値が入るのは `.env` と Vercel の2か所だけで、GitHub に載るのは設定名と練習用の値の見本です。実線は値が実際に使われる向き、点線は名前をそろえる作業を表します。
...
- 「ダッシュボードへ入る」を押すと、Day 02 で作った自分用メッセージが見える
...
上のキャプチャは Day 02 で載せたものと同じ画像です。`npm run dev` でも `npm start` でも、公開した URL でも、画面の見た目は変わりません。変わるのは届く範囲だけなので、どこで開いても同じ絵になります。別々に撮って並べると「何かが違うはず」と探させてしまうので、同じ画像を指しています。
...
まだ作りかけですが、URL を持てたのがうれしいです。
次は Day 05 からログイン機能を作ります。
...
| エラー / 問題 | 原因 | 解決方法 |
|--------------|------|---------|
| `npm run build` が `Type error:` で止まる | 型の書き間違いが、開いていないページに残っていた。`npm run dev` は型検査の代わりにならないので気付けない | 赤い行のうち**いちばん上**だけを読む。`Type error:` の1つ上の行にファイル名と行番号、`Type error:` の後ろに理由、その下にコードの抜粋が出る。そこを直して `npm run build` をやり直す |
...
A. 確認する範囲が違います。`dev` は書き換えるたびに表示を作り直しますが、型検査の代わりにはなりません。開けたページにも型のずれが残ることがあります。`build` は本番用の完成品を1回だけ作り、その途中で全ファイルの型をまとめて確認します。Vercel が公開時に走らせるのも、この `build` と同じ処理です。だから公開前に手元で通しておくと、失敗する場所を先に見つけられます。
