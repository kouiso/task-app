...
| 公開先だけビルドの結果が手元と違う | Node のバージョンが手元と公開先でそろっていない | `package.json` の `engines.node` または Vercel の Project Settings で、公開先の Node.js バージョンを確認する。Day 03 の `.node-version` はローカルのバージョン管理ツール向けで、Vercel の指定には使わない |
