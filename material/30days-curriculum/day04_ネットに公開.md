...
このコマンドは、Day 01 から使ってきた `npm run dev` とは別の道を通ります。`dev` は書き換えるたびに画面へ反映する速さを優先するので、使っていない変数や型のずれが少し残っていても動き続けます。`build` は本番用の完成品を1回だけ作る処理なので、そこを見逃しません。そして Vercel が公開のときに走らせるのも、この同じ `build` です。手元で通ることを先に確かめておけば、Vercel で同じ場所につまずく可能性をあらかじめ減らせます。
