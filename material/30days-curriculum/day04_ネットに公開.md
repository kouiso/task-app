...
`build` が見てくれるのは型です。このプロジェクトは使っていない変数も型エラーとして扱う設定にしてあるので、消し忘れがあれば `build` はそこで止まります。書き方のクセや並び順など、型では表せない部分を見るのが `npm run lint` の役目なので、そちらは別で走らせてください。
...
この5行が順に流れるのは、build が決まった順番で進むからです。ソースの変換、型の確認、ページごとの下ごしらえ、最後に最適化、という並びになっています。だから途中で止まったときは、最後に出た行の名前がそのまま調べる場所を教えてくれます。`Linting and checking validity of types` の行で止まれば、まず型の書き間違いを疑います。`Generating static pages` まで進んでいたなら、型ではなくページ側の処理でエラーが起きています。
...
この2つは、証明している中身が違います。シークレットウィンドウでわかるのは、保存済みのログイン情報やキャッシュ（一度読んだデータを手元に貯めておく仕組み）に頼らず表示できることです。モバイル回線のスマホでわかるのは、自宅のネットワークの外からでも届くことです。前者だけでは、同じ回線の中でしか見えない状態と区別が付きません。両方そろって、ようやく他人の環境でも開けると言い切れます。
