...
### 本物のシークレットを Vercel に入れない

Day 03 では `.gitignore` と `.env.example` の役割を確認しました。本物の値は送らず、見本だけを送るという線引きです。Vercel の Environment Variables も同じ考え方です。今日は環境変数を追加しないので問題ありません。今後 API キーを扱うときは、その値が公開してよいものか、必ず確認します。なお `NEXT_PUBLIC_` で始まる環境変数はブラウザにも渡ります。秘密の値には使いません。
