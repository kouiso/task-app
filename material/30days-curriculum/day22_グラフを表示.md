...
統計カードは合計や割合は分かりますが、
どのステータスにタスクが偏っているかまでは読み取れません。
グラフにすると、その偏りが一目で分かるようになります。
...
props が5個以上になったら「まとめてオブジェクトで渡す」か「Pick で型から切り出す」を検討します。バラバラの primitive props は管理コストが高いです。
