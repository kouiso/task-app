...
| `// filepath:` | TypeScript のコード。`<a` から `>` までのタグの設定を書く場所もここです |
| `{/* filepath: */}` | `>` を閉じたあと、次のタグまでのあいだ |

3つとも、その言語で「コメント」として扱われる書き方です。
コメントは、書いても画面には出ませんし、動きも変わりません。

画面の見た目を書く部分（JSX）が2つに分かれているのは、
場所によってコメントの書き方が変わるためです。

`<a` と `>` のあいだは、タグの設定を書く場所です。
ここに置いた `//` はコメントとして読み飛ばされ、画面には出ません。

`>` を閉じたあと、次のタグまでのあいだは、画面に文字が出る場所です。
ここに `//` を置くとコメントにならず、そのまま画面に表示されます。
逆に、タグの設定を書く場所へ `{/* */}` を置くとエラーになり、画面が出なくなります。
...
>
> `Local:` の番号が `http://localhost:3001` のように 3000 以外になることもあります。3000 番を別のプログラムが使っているときに Next.js が空いている番号へ移るためで、失敗ではありません。ブラウザで開くのは、そのときターミナルに出ている `Local:` の URL です。この先の説明では `http://localhost:3000` と書きますが、番号が違っていたら自分の画面に出ている番号に読み替えてください。
>
