...
todo ここ変なので修正1文目が日本語の言い回しではない2分目何言ってるか分からん3文目いらなくね
...
todo go foundationっていきなり出てきたけどなに
...
何もない状態から進める都合上、
いくつか前提は必要になります。
todo 初心者になんの説明もなくnode npm dockerとか言ってもわかるわけないダウンロード方法から説明するべきでは
### 必須
...
ここが Day 01 の肝です。
todo そもそもcreate-next-appしたことない人にじゃなくてっていっても伝わらん
...
```
todo なぜここにポイントを置いた？出力テキストも写経するのかと勘違いする
...
ここで一個、注意して欲しい点を話しておきます。
todo 秘密値？シークレットキーでは なぜいけないのか言及なし
`JWT_SECRET` に本番で使う秘密値をそのまま貼って、
公開場所に出してしまうのはいけません。

今日は `.env.example` を眺めるだけで十分です。
本物の秘密値は、あとで `.env` を作る段階で自分の手元だけに置きます。
...
todo タイトルがAIくさい なんだ安心して良い理由て
### ここで一回安心してよい理由

この時点で、
Node.js と npm と Next.js と Tailwind の土台はちゃんと動いています。

todo 初心者に壊すというと物理的な画面だと思わね
つまり次に画面を壊したとしても、
「最初は動いていた」状態に戻れます。
これは地味ですが、とても大事です。
...
todo 確認ポイントって何者？いらなくね

### 4-2. `page.tsx` を最初の画面に置き換える

todo 顔ってなんだよaiくさい
次に、
トップページを自分のアプリの顔に変えます。

今日のテーマはこれ。
todo なぜさっきからSNSが出てくるのか
