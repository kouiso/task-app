...
| `// filepath:` | TypeScript のコード。`<a` から `>` までのタグの途中もここです |
...
`<a` と `>` のあいだは TypeScript の続きなので `//` が通じます。
`>` を閉じたあと、次のタグまでのあいだは画面に文字が出る場所なので、
`//` を置いてもコメントにならず、そのまま画面に表示されます。
逆に、タグの途中へ `{/* */}` を置くとエラーになり、画面が出なくなります。
