...
ようこそ！ 30日間カリキュラムの第1日目です。今日はこれから30日間で作り上げていくタスク管理アプリを、あなたのパソコンで実際に動かすところまで進めます。最初のセットアップは少し手順が多く感じるかもしれませんが、ひとつずつ進めれば大丈夫です。

今日の終わりには、ブラウザに自分だけのアプリが表示される達成感を味わえます。
...
| 2 | VS Codeをインストールする | 5分 | なし | VS Codeが起動する |
| 3 | VS Codeの拡張機能を入れる | 5分 | なし | 4つの拡張機能が入っている |
| 4 | Docker Desktopをインストールする | 7分 | なし | `docker --version` でバージョンが表示される |
| 5 | プロジェクトをクローンする | 5分 | なし | `task-app` フォルダが作成される |
| 6 | VS Codeでプロジェクトを開く | 3分 | なし | VS Codeにファイル一覧が表示される |
| 7 | Node.jsをセットアップする | 7分 | なし | `node -v` で `v25.6.1` が表示される |
| 8 | 環境変数を設定する | 5分 | .env | `.env` ファイルが作成される |
| 9 | 依存関係をインストールする | 5分 | なし | `node_modules` フォルダが作成される |
| 10 | データベースを起動する | 5分 | なし | PostgreSQLコンテナが起動する |
| 11 | データベースをセットアップする | 5分 | なし | テーブルとサンプルデータが作成される |
| 12 | 開発サーバーを起動する | 5分 | なし | ターミナルに「Ready」と表示される |
| 13 | ブラウザでアプリを確認する | 5分 | なし | ログイン画面が表示される |
...
> 📸 `mise --version` の出力にバージョン番号が表示されていることを確認してください。
...
📝 **学んだこと**: VS Codeは世界中の開発者が使っている無料のコードエディターです。

---

### Step 3: VS Codeの拡張機能を入れる（5分）

🎯 **ゴール**: このプロジェクトの推奨拡張機能4つが入っている状態にする。

> 💡 **例え話**: 拡張機能は「VS Codeのプラグイン」です。料理の道具に例えると、VS Code本体は包丁、拡張機能は皮むき器や計量カップのような専用道具です。
...
### Step 4: Docker Desktopをインストールする（7分）
...
> 📸 ターミナルに `v25.6.1` と表示されていることを確認してください。
...
> 📸 ターミナルに `Your database is now in sync with your Prisma schema.` と表示されていることを確認してください。
...
| webpack | Next.jsのデフォルトバンドラー（コードをまとめて配信する仕組み） |
...
> 📸 ログイン後にダッシュボード画面が表示され、タスク一覧が見えていることを確認してください。
