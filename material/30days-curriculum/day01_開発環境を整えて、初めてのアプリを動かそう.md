...
このカリキュラムでは、30日かけて自分専用のタスク管理アプリを作ります。今日はその1日目です。
...
- **macOS**: `command + スペース` で Spotlight を開き、`ターミナル` と入力して Enter
- **Windows (WSL2)**: スタートメニューで `Ubuntu` を検索して起動
...
PostgreSQL の用意のしかたは2通りあります。Docker Desktop から起動する方法と、パソコンに直接インストールした PostgreSQL を使う方法です。どちらか片方が使える状態になっていれば、今日のスクリプトは問題なく進みます。初めての場合は、用意がかんたんな Docker Desktop をおすすめします。
...
> - `unzip ...`: 配布 ZIP を展開します
> - `cd task-app`: 展開してできた `task-app` フォルダに移動します
...
- `pwd` の結果が `~/workspace/task-app` になっている（パスの先頭部分は、使っているパソコンによって変わります）
...
派手にする必要はありません。それでも「Next.js の初期画面のまま」からは卒業して、自分のプロダクトらしい1枚目に仕上げます。
...
まず `src/app` の中に `dashboard` フォルダを作ります。その中に `page.tsx` を新しく作り、次の内容をそのまま入力しましょう。
...
- Day 02 では、この `Hello Task-App` のダッシュボードに自分のメッセージを追加していきます
...
Day 01 のポイントはここです。「動く」と「見た目が整っている」の両方を、初日のうちに手に入れられました。
