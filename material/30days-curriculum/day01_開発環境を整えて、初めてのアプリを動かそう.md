...
| `npm install` でエラーが出る | Step 5 | Node.jsのバージョンが v24 か確認 |
...
Node.js公式サイト（ https://nodejs.org/ ）にアクセスし、**v24 LTS**をダウンロードしてインストーラーを実行してください。インストール時の設定はすべてデフォルトのままで問題ありません。
...
| コマンド | 意味 | 期待する結果 |
|---------|------|------------|
| `node -v` | Node.jsのバージョンを表示 | `v24.x.x` と表示される |

✅ **確認ポイント**:
- `v24` で始まるバージョン番号が表示されている
...
| Prettier | コードの自動整形 | コードの見た目を統一してくれる |
...
【スクリーンショット: task-appフォルダの中身が表示された状態】
...
| `NEXTAUTH_URL` | アプリのURL | お店の住所 |
| `NEXTAUTH_SECRET` | セッション暗号化の鍵 | 会員カードの暗号鍵 |
...
【スクリーンショット: Docker Desktopでコンテナが実行中の状態】
...
【スクリーンショット: ターミナルに「Ready」が表示された状態】
...
| 項目 | 値 |
|------|-----|
| メールアドレス | シードデータで設定されたメール |
| パスワード | シードデータで設定されたパスワード |
