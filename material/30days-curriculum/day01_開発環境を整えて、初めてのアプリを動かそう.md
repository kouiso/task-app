...
今回は空のディレクトリから開発環境を立ち上げます。ブラウザに最初の画面を表示するところまで進めます。

今日用意するのはタスク管理ツール Linear の配色を参考にした画面です。

## 今日のゴール

空のディレクトリから `task-app` の土台を起動して `http://localhost:3000` に最初の画面を表示します。そのあとで配色やフォントなどの見た目の設定（design token）を整えます。
...
> このカリキュラムは macOS を基準に説明します。Windows は「WSL2」で動く Ubuntu を使います。WSL2 が無い場合は先に [Microsoft の公式手順](https://learn.microsoft.com/ja-jp/windows/wsl/install) に従って WSL2 と Ubuntu を入れてください。以降のコマンドはすべて Ubuntu のターミナルで実行します。Ubuntu 22.04 を直接使っている場合も Linux 用と書かれた手順を選びます。
...
Step 2 の10分には npm のダウンロードと PostgreSQL の起動を待つ時間が含まれます。Step 4 の25分はコードを16ブロックに分けて写す目安です。調べものやエラーの修正にかかる時間は含みません。
...
> - `mise trust` と `mise install`: mise を使う場合だけ配布物の Node.js 設定を許可してインストールする
...
8. Prisma（データベースを TypeScript から扱うための道具）のスキーマ（データベースにどんな表と項目を作るかを書いた設計図）と Docker Compose を配置
...
スクリプト（`.sh` ファイル）を直接実行するには実行権限が必要です。`chmod +x` はその権限を付けるコマンドです。次の行では `bash` にファイルを渡します。`bash` はスクリプトの命令を上から順番に実行するプログラムです。
...
下の見本は今日使う行だけを抜き出しています。実ファイルには Docker のポート番号や本番用 URL などの設定もあります。これらは後の Day で扱います。
...
| `// filepath:` | ふつうのコードの中と `<a` から `>` までのタグの設定を書く場所 |
...
コードは3つのまとまりに分かれています。先にそれぞれの役割を確認しましょう。
...
`@import "tailwindcss";` は Tailwind CSS 本体を読み込む宣言です。`@custom-variant dark` は `.dark` クラスが付いたときのスタイルを指定する準備です。`@theme inline { ... }` にクラス名と変数の対応を書きます。`--font-sans` と `--font-mono` は本文とコードに使うフォントを複数候補から順に選ぶ指定です。まだ `@theme` の括弧は開いたままです。続けて `--color-` 系の行を書きます。

**確認ポイント**: `monospace;` の行まで書けたことを確かめます。
...
`--color-primary: hsl(var(--primary));` は `bg-primary` などで使う色を `--primary` に結び付けます。
...
`export default function HomePage()` はページに表示する部品の宣言です。Next.js は `src/app/page.tsx` をトップページ（`/`）として使います。`className` の `bg-background` や `text-muted-foreground` は `globals.css` に登録した token を参照します。色の変更は token の値に反映させます。
...
3枚の小さなカードと左側の大きなカードを閉じて、右側の列を開く `<div>` まで書けました。次のブロックはこの列の中身です。先頭の `<article>` に付く `id="today-goals"` はこの場所の目印です。リンクの `href="#today-goals"` と同じ名前にするとクリック時にここまでスクロールします。

**確認ポイント**: `<div className="space-y-4">` の次に `<article` と `id="today-goals"` が続くことを確認します。
...
{/* filepath: src/app/page.tsx（同じファイルの続き） */}
            ...
            >
              ...
              <ul className="mt-4 space-y-3 text-sm leading-7 text-muted-foreground">
                <li className="rounded-2xl bg-secondary px-4 py-3">プロジェクトを作って、タスクを登録する</li>
                <li className="rounded-2xl bg-secondary px-4 py-3">期限と優先度で並べ替える</li>
                <li className="rounded-2xl bg-secondary px-4 py-3">進み具合をレポートで見る</li>
                <li className="rounded-2xl bg-secondary px-4 py-3">メンバーを招いて一緒に使う</li>
              </ul>
            </article>
...
Day 02 の予告文のあとに `</section>` から `</main>` までの閉じタグを書きます。開いた順とは逆に内側から閉じて `HomePage` の `return` を終えます。JSX は開いたタグを閉じる必要があります。保存後にエラーが出たら閉じタグの対応を確認します。
...
この例は `bg-[#0f1021]` や `text-[13px]` のように値を直接書いています。`#0f1021` だけでは背景用の色なのかが分かりません。複数の画面で同じ色を使うと変更時にそれぞれの値を探す必要があります。続きにはボタンの指定も出てきます。
...
（3）ヘッダーとボタンの角丸が大きくなることを確認します。`rounded-[28px]` のメインカードは変わりません。予想と違う場合は明るいテーマの `:root` を編集したか確かめます。
...
A. `--radius` を参照する角丸が大きくなります。`--radius-sm` から `--radius-xl` はどれもこの値が基準です。`--radius-lg` は値をそのまま使います。ほかの3つは `calc()` で増減させます。今日の画面では `rounded-xl` のヘッダーと `rounded-lg` のボタンが変わります。`rounded-[28px]` のメインカードは直接指定した値なので変わりません。
