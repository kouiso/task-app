...
- Ubuntu を直接使う場合と Windows の WSL2 では `/home/あなたのユーザー名/workspace/task-app` のような形になる
...
次は、セットアップ後の `task-app` を Ubuntu で開いた画面です。左側に `src` と `package.json` が見えています。OS や表示言語が違っても、この2つを確認します。

![セットアップ後のtask-appをVS Codeで開き、srcとpackage.jsonが並んだ状態](./screenshots/day01-vscode-open.png)
