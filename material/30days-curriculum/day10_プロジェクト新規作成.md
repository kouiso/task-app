...
スクリーンショット: 下の画像は、Step 5 の作例と同じく名前に「ポートフォリオサイト」と入れて作ったときの一覧です。名前は本文で指定していないので、自分が入れた名前のカードが増えていれば同じ結果です。

![一覧に新しいカードが1枚増えた画面。赤枠の中が、いま作ったプロジェクト](./screenshots/day10/project-list-after-create.png)
...
スクリーンショット: 下の画像は、プロジェクト名を空のまま「作成」を押したときのエラー表示です。
