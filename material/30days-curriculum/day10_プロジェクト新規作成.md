...
G の「キャッシュ更新」が H の手前に置いてあるのにも理由があります。E の保存が成功した時点では、画面が抱えている一覧のデータはまだ作成前のままです。ここで取り直しておかないと、H でダイアログを閉じても I の「一覧に新プロジェクト表示」までたどり着けません。今日はこの D と G の2か所を、それぞれ Step 0 と Step 7 で手を動かして埋めます。
...
色の欄が Step 4 の名前欄と違うのは、`placeholder` を持たない点です。色には「まだ何も入っていない」という状態がなく、`DEFAULT_PROJECT_COLOR` が最初から入っています。読者がカラーピッカーに触らなくても、必ず何かの色を持ったまま送信されます。だから Step 0 のサーバー側でも、`color` には既定値を用意する `.default(...)` だけを付け、空を弾く `.min(1)` は付けていません。
