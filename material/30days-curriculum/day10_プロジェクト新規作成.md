...
`void utils.project.getAll.invalidate()` を追加します。
...
まず名前欄が空でないか確認します。Day 10 の `createMutation` には `onError` を書いていないため、サーバー側で失敗しても画面には何も出ません。ブラウザの開発者ツールで Network タブを開き、失敗した `/api/trpc/project.create` の応答を確認してください。
