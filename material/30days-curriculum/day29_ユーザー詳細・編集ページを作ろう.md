...
Day 22 で学んだ `useEffect` パターンと同じく、`error` が変化したときだけトースト表示します。次に早期リターンを書きます。
...
Step 3 と同じパターンで `useParams` → `useQuery` → 早期リターンを実装します。`useState` の初期値を空文字にするのは、React の制御コンポーネントを `undefined` で初期化すると警告が出るためです。
...
早期リターン（ローディング → 未発見）は Step 3 と同じパターンです。その後に権限チェックを追加します。
...
```tsx
// filepath: src/app/user/[id]/edit/page.tsx（権限チェック）
  const isAdmin = currentUser?.role === USER_ROLE.ADMIN;
...
`useEffect` でサーバーデータが届いたらフォームに反映する、**サーバーデータとフォーム状態の同期**パターンを実装します。
...
`user` データは非同期に取得されるため、コンポーネント表示時はまだ `undefined` です。`[user]` 依存配列により、データ到着時に `setFormData` が自動実行されフォームが埋まります。
...
`[name]: value` の `[]` は「変数の値をキー名として使う」JavaScript の構文（computed property name）です。これにより1つのハンドラで全テキスト入力を処理できます。
...
- アバターURLを入力するとプレビューがリアルタイムで変わる（`formData.avatar` が `AvatarImage` の `src` に直結しているため）
...
**サーバー側のアクセス制御**: 一般ユーザーが自分の `role` や `isActive` を変更しようとすると、サーバーが `FORBIDDEN` エラーを返します。今回はUIでは全フィールドを表示し、サーバー側のガードに任せるシンプルな設計にします。
...
`onValueChange` の `value` は `string` 型ですが、`formData.role` は `UserRole` 型が必要です。`isUserRole` 型ガードで安全に変換します（`as UserRole` は使いません）。
...
| `useParams()`が`undefined`を返す | App Routerの動的ルートでファイル配置が間違っている | `app/admin/users/[id]/page.tsx`のパス構造を確認する |
| フォームの初期値が空になる | `useForm`の`defaultValues`にデータを渡していない | `defaultValues`にAPIから取得したユーザーデータを設定する |
| 権限チェックが効かない | `session.user.role`の比較で文字列が一致しない | `role === "ADMIN"`のように大文字のEnum値で比較する |
| 更新後に古いデータが表示される | キャッシュの無効化が漏れている | `onSuccess`内で`utils.user.getById.invalidate({ id })`を呼ぶ |
| `FORBIDDEN`エラーがコンソールに出る | 一般ユーザーが管理者専用フィールドを送信している | フロント側で`role !== "ADMIN"`のとき該当フィールドを`disabled`にし、送信データからも除外する |
