...
真偽値を2段に分けてあるのが要点です。`canEditUser` はページへ入れる権限、`canManageAccount` はロールとアクティブ状態を触れる権限です。後者に `!isOwnProfile` が入っているので管理者が自分の権限を自分で下げる操作が塞がれます。`enabled` で権限を確かめてから取得するため入れない人の画面では通信そのものが起きません。
