...
URLパラメータ取得 → データ取得 → 早期リターンの流れで実装します。`useState` の初期値を空文字にするのは、React の制御コンポーネントを `undefined` で初期化すると警告が出るためです。
...
✅ **確認ポイント**: ファイルを保存してエラーが出ないことを確認してください。まだフォームのJSXは書いていないので画面に変化はありません。
...
`[name]: value` の `[]` は「変数の値をキー名として使う」JavaScript の構文（computed property name）です。

| `name` 属性の値 | `[name]: value` の展開結果 |
|-----------------|--------------------------|
| `"name"` | `{ name: "入力値" }` |
| `"avatar"` | `{ avatar: "入力値" }` |

これにより1つのハンドラで全テキスト入力を処理できます。

CardContent 内のフォームを書きます。アバタープレビューと名前入力欄です。

> 💡 `handleSubmit` と `updateUser` は
...
// filepath: src/app/user/[id]/edit/page.tsx
  ...
  // 非管理者は role/isActive を送信すると FORBIDDEN になるため除外
...
✅ **確認ポイント**: ファイルを保存してエラーが出ないことを確認してください。`handleSubmit` と `updateUser` が定義されたことで、Step 8 で書いた `<form onSubmit={handleSubmit}>` が動作するようになりました。
