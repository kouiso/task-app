...
ここで誤解しやすいのは、速さの理由をひとつに決めてしまうことです。理由は2つあります。1つは実行環境そのものです。Edge Runtime は使える機能を絞った軽い環境で、起動が速いという特徴があります。もう1つが距離です。Next.js の公式ドキュメントは、この実行環境について「edge で動かすことを必須とせず、単一リージョン（1か所の地域だけで処理する構成）のサーバーでも動く」と書いています。つまり実行環境の軽さと、拠点をどこに置くかは別の話です。
...
回数を決めているのが matcher です。`npm run dev` の間は、開発専用のリクエストが何度も飛びます。まず画面の更新を知らせるリクエストです。webpack を使う設定なら `/_next/webpack-hmr`、Turbopack なら `/_next/turbopack-hmr` になります。もう1つはエラー画面が使う `/__nextjs_original-stack-frame` です。いまの matcher はこれらを除外していません。ファイルを保存するたび、そしてエラーが出るたびに、出入りの回数は増えます。
