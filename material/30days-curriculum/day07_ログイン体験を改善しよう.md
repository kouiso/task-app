...
`bcrypt.hash` の第2引数の `10` は、変換を何回繰り返すかの目安です。数を増やすほど1回の変換に時間がかかり、総当たりで試す側も同じだけ待たされます。小さくすると登録は速くなりますが、盗まれたときに試される回数も増えます。大きくしすぎると、登録とログインのたびに読者を待たせます。`10` はその折り合いとして広く使われている値です。
