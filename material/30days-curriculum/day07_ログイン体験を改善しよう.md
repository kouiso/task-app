...
matcher に当てはまったリクエストは、すべて middleware を通ります。中身は JWT の検証だけなので、1回の処理はミリ秒に満たない時間です。それでも体感の速さは変わります。**middleware を置く場所が、デプロイ先によって変わるためです。**
...
ここで誤解しやすいのは、Edge Runtime そのものが速さを生むと考えてしまうことです。Next.js の公式ドキュメントは、この実行環境について「edge で動かすことを必須とせず、単一リージョンのサーバーでも動く」と書いています。本番で速さの差を生むのは処理の重さではありません。拠点が1つか、世界中にあるかの違いです。

**ところが、手元の `npm run dev` では話が変わります。** ここでも middleware は Edge Runtime で動きますが、本物ではありません。Next.js が Node.js の中に Edge を真似た入れ物を用意し、その中で動かしています。公式ドキュメントは、この真似る処理そのものに手間がかかると書いています。middleware を1回通るたびに、その入れ物へ出入りする手間を払います。

だから重く感じるのは、JWT の検証が重いからではありません。**入れ物への出入りが、リクエストのたびに挟まるためです。** 公式ドキュメントがこの手間に触れているのは、ローカル開発の文脈です。Day 30 で公開する Vercel では、拠点に置かれた実行環境がそのまま動きます。

回数を決めているのが matcher です。`npm run dev` の間は、開発専用のリクエストが何度も飛びます。画面の更新を知らせる `/_next/webpack-hmr` や、エラー画面が使う `/__nextjs_original-stack-frame` です。いまの matcher はこれらを除外していません。ファイルを保存するたび、そしてエラーが出るたびに、出入りの回数は増えます。

開発中だけ重いと感じたときは、まず matcher が何を通しているか確かめてください。本番では飛ばないリクエストなので、公開したあとの速さには影響しません。

matcher を絞る話も、ここにつながります。当てはまる URL が多いほど、この判定を通る回数も増えます。画像やスタイルを外してあるのは、そのためです。
