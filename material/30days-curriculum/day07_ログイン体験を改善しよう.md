...
ここで誤解しやすいのは、Edge Runtime そのものが速さを生むと考えてしまうことです。Next.js の公式ドキュメントは、この実行環境について「edge で動かすことを必須とせず、単一リージョンのサーバーでも動く」と書いています。速さの差を生むのは処理の重さではありません。拠点が1つか、世界中にあるかの違いです。

ローカルの開発サーバーが重く感じるのは、また別の理由です。`npm run dev` の間は、開発専用のリクエストが何度も飛びます。画面の更新を知らせる `/_next/webpack-hmr` や、エラー画面が使う `/__nextjs_original-stack-frame` です。いまの matcher はこれらを除外していません。その全部が middleware を通ります。1回ずつ JWT の検証を受けます。

ファイルを保存するたび、そしてエラーが出るたびに、検証の回数は増えます。本番では飛ばないリクエストなので、公開したあとの速さには影響しません。開発中だけ重いと感じたときは、まず matcher が何を通しているか確かめてください。
