...
| 概念 | 読み方 | 役割 | 例え |
|------|--------|------|------|
| `export default function` | — | コンポーネント（画面の部品）を定義して外から使えるようにする宣言 | 「この部品を他のファイルから呼び出せるようにする」宣言書 |
| `className` | クラスネーム | HTML の `class` と同じ。React では `className` と書く | 見た目の指定タグ。Tailwind のクラス名をここに書く |
| `const` | コンスト | 変更できない変数の宣言。値を入れたら書き換えない | 「この箱の中身はもう変えへんで」という約束 |
| TypeScript の型 | — | 変数にどんな値が入るか事前に決めておく仕組み | 「この箱には文字しか入れたらあかん」という注意書き |
