...
もう1つ、`<h2>` の中の `{ownerName}` に注目してください。JSX は HTML によく似た React の書き方です。このタグの中で `{ }` を使って変数名を囲むと、その変数の中身が画面へ差し込まれます。`こんにちは、{ownerName}さん。` は、実際の画面では `こんにちは、Taroさん。` と表示されます。これが React で「用意した値を画面に出す」いちばん基本の形です。
...
また、`buildMainMessage` が返している `` `${greeting}、${owner.name}さん。…` `` は、バッククオート（`` ` ``）で囲んだ文字列です。この中では `${ }` で囲んだ部分が変数の値に置き換わります。`+` で文字列を何度もつなぐより、文章の形のまま値を差し込めるので読みやすくなります。
...
3枚のカードは `focusCards.map((card) => (...))` で作っています。`map` は配列の要素を1つずつ受け取り、それぞれを JSX に変換する書き方です。`focusCards` に要素が3つあるので、カードも3枚できます。`key={card.label}` は、React がどのカードがどれなのかを見分けるための目印で、`map` で要素を並べるときは付けるのが決まりです。
