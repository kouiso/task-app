...
分割されたコードの途中では括弧やタグが閉じていません。

**解決方法**

Step の最後まで書きます。エラーが残れば閉じタグと貼り付け漏れを確認してください。

#### かっこの数が合わないと言われる

**原因**

エラー行より上でタグや `}` を閉じ忘れています。

**解決方法**

エラー行から上へたどります。開いたタグと閉じたタグを逆順に照合してください。

#### `Cannot assign to 'currentHour' because it is a constant.`
...
再代入をやめます。必要な場合だけ `let` を使います。

#### `Argument of type 'string' is not assignable to parameter of type 'number'.`

**原因**

`number` の引数に文字列を渡しています。

**解決方法**

数値なら引用符を外します。文字列を数値へ変える場合は `Number('9')` を使います。

#### 公開後のあいさつが変わらない

**原因**

ビルド時に作った HTML を配っています。

**解決方法**

今日は固定表示として扱います。閲覧者ごとに変えるにはブラウザ側の時刻で計算する処理が必要です。
...
`dashboardOwner` に値をまとめます。JSX は `{ }` で参照します。
