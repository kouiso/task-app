...
ここが今日の主役です。`<article>` の中を上下2段に分け、下線を引いた上の段にあいさつと集中テーマ、`bg-secondary` の下の段に応援の一言を置いています。段ごとに背景色を変えると、同じカードの中でも見出しと添え書きが別の役目だと目で分かります。
...
`<h2>` に入っているのが、さきほど組み立てた `mainMessage` です。Step 1 では文の形を JSX に直接書いていましたが、いまは出来上がった1本の文を受け取るだけになりました。朝に開けば「おはよう」、夜に開けば「こんばんは」と、この行を書き換えずに表示が変わります。
...
先に開いていた小さな枠と下段の `<div>` を閉じ、`</article>` で主役のカードを閉じます。そのあとの `<aside>` は Step 1 と同じ位置にある右側の2枚で、中の文章だけを今日の内容へ差し替えています。
...
いま出ているあいさつは、開いた時刻だけで決まります。境目の動きまで確かめたいときは、`const currentHour = new Date().getHours();` の行を一時的に `const currentHour = 20;` へ書き換えて保存してください。あいさつが「こんばんは」に変われば、`getGreetingByHour` の分かれ道が働いています。確かめたら元の行に戻します。
...
あいさつを返す2つの関数は Step 2 とまったく同じで、書き写す位置だけが変わります。Step 3 で新しいのは `focusCards` です。`FocusCard[]` は `FocusCard` の形をした要素が並ぶ配列という意味で、1枚目に `Owner` の情報を入れています。
...
2枚目と3枚目を足して、`focusCards` が3要素の配列になりました。並び順がそのまま画面の左から右の順になるので、カードを入れ替えたいときは JSX ではなくこの配列の順番を動かします。
...
右上のラベルが `Day 02 Progress` から `Personalized Message Ready` に変わっています。文字を差し替えただけですが、画面を開いた自分に「今日のぶんは入った」と伝える合図になります。

`<h2>` に `max-w-4xl` が増えているのも Step 2 との違いです。あいさつの文は名前と集中テーマを含むので長くなりやすく、幅を決めずに置くと画面の端まで1行で伸びます。上限を決めておけば、長い文でも決まった幅で折り返します。
...
`key={card.label}` は、React がどのカードがどれなのかを見分けるための目印です。付け忘れても表示はされますが、ブラウザの開発者ツールに警告が残ります。中で読んでいるのは `card.label`・`card.value`・`card.description` の3つで、`FocusCard` 型で決めた3項目とそろっています。だから表示する項目を増やしたいときは、型・配列・この中身の3か所を合わせて直します。
...
下段を閉じて主役のカードを閉じ、右側の `<aside>` に入ります。ここは Step 2 と同じ2枚で、文章だけ Day 02 の締めに合う内容へ変えています。
...
3枚目の `<article>` を足して、`</aside>` から `</main>` までを閉じます。右の列だけ2枚から3枚に増えましたが、隙間は `space-y-4` が引き受けてくれるので、余白の調整は要りません。
...
型と関数と配列という3つの道具が、今日はじめて1つのファイルにそろいました。`DashboardOwner` が持たせる情報の形を決め、`buildMainMessage` が文の組み立てを引き受け、`focusCards` が並べる順番を持っています。Step 1 の状態と見比べると、画面に出る文字はどれも JSX の外側で決まっていて、JSX 側には `{ }` で受け取る場所しか残っていません。この分かれ方ができていれば、明日以降ここへタスク件数のような本物のデータを流し込むときも、差し替えるのは値を作る側だけで済みます。
...
Before でまず目に入るのは、1行目の `'use client'` です。この宣言が付いたファイルは、中身がブラウザへ送られてそこで動く扱いになります。Day 02 の画面は文字を並べるだけで、押す場所や入力する場所はありません。それでもこの1行があると、下に続く見た目のコードまでまとめてブラウザへ運ばれます。2行目で `useMemo`（計算した結果を覚えておいて、材料が変わらなければ計算をやり直さない React の仕組み）を取り込めるのも、この宣言があるからです。
...
ここから先は、見た目を組み立てる JSX が続きます。押す場所も、値が変わって表示が動く場所もありません。それでも1行目に `'use client'` があるので、この部分もブラウザ側で組み立てる扱いになります。読者のブラウザへ届く JavaScript の量は、ここに書いたタグの数だけ増えます。
...
After の1行目に `'use client'` はありません。`useMemo` の取り込みも消えています。それ以外は Before と同じで、`type` の並びも `dashboardOwner` の中身も変えていません。直したのはファイルの入り口の2行だけ、というのがこの読み比べの要点です。この状態のページは、サーバー側で HTML を組み立ててからブラウザへ送られます。
...
`mainMessage` の行を Before と見比べてください。`useMemo` が外れて、`buildMainMessage(dashboardOwner, currentHour)` を呼ぶだけになりました。この関数はページを組み立てるときに1回動くだけなので、結果を覚えておく相手がいません。覚えておく仕組みを外すと、`[currentHour]` の書き漏らしを心配する場所も消えます。
...
見た目の部分は Before から変わりません。同じタグ、同じ `className`、同じ差し込みです。それでも読者のブラウザへ届くものは違います。Before ではこの JSX を組み立てる JavaScript が送られ、After では組み立て終わった HTML が送られます。見え方が同じだからといって、送られている中身まで同じではありません。
...
`map` でカードを3枚作る処理も、そのまま置いています。この配列はサーバー側で回るので、3枚のカードは HTML になった状態でブラウザに着きます。画面が出るまでに JavaScript の読み込みを待つ必要がありません。Day 02 のように出す内容が決まっている画面では、この差がそのまま表示の速さに出ます。
