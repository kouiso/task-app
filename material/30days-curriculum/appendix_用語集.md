...
ユーティリティファーストのCSSフレームワーク。`className="text-red-500 font-bold"`のように、クラス名を組み合わせてスタイルを適用します。多くのスタイルをクラス名で指定できます。この教材では、Tailwind の読み込みと共通設定に `globals.css` も使います。
...
TypeScriptで型安全なAPIを構築するライブラリ。フロントエンドとバックエンドで型を共有し、引数や戻り値の型の不一致を開発中に見つけやすくします。外部から届く値の検証や権限確認は別に必要です。
...
リクエストが処理される前に実行される処理。ビルの入り口の警備員のように、`config.matcher` で指定した対象ルートへのリクエストをチェックし、ログインしていなければログイン画面へ送り返します。
...
JWTを生成・検証するライブラリ。Web 標準の暗号 API を使い、Next.js の Edge Runtime でも動作します。
...
Web 標準の API を中心に使う、Next.js の実行環境です。Node.js の全機能は使えません。実行される地域はデプロイ先の設定で決まり、常にユーザーの近くで実行されるとは限りません。このカリキュラムではNext.jsのミドルウェアで使います。
