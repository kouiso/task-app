...
### コンポーネント
UIの再利用可能なパーツ。例えば「ボタン」「カード」「ナビゲーションバー」などです。一度作っておけば、ほかの画面から同じ部品を何度でも使い回せます。
...
### JSX / TSX
JavaScriptの中にHTMLのような構文を書ける拡張記法。TSXはTypeScript版です。見た目はHTMLですが、実際にはJavaScriptのオブジェクトに変換されます。
...
### Turbopack
Next.js内蔵の高速バンドラー。開発サーバーの起動や、コードを変更したときの画面への反映が速くなります。
