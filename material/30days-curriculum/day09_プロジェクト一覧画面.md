...
この図で目を留めてほしいのは、いちばん下の矢印です。ページが受け取るのは `data` だけでなく、`isLoading` と `error` も一緒です。リクエストはページを出てサーバーとデータベースまで届き、そこから折り返してきます。だから画面が現れた瞬間、`data` はまだ `undefined` のままです。Step 3 で読み込み中の表示に切り替えるのは、この空白の時間があるからです。データベースにつなぐのはサーバー側だけです。ページ自身は `prisma` を知りません。Day 07 で作った認証 API も同じ道を通ります。
...
> 今日は `src/app/project/page.tsx` を **ゼロから書いていく** 方式で進めます。すでにファイルがある場合は一旦中身を空にして、ステップごとにコピペしてください。
...
一覧画面には、サーバーが持っているプロジェクトを画面まで運んでくる入口が必要です。その入口を、今日は自分の手で1つ作ります。Day 07 で認証 API（`auth.ts`）を作り直したのと同じ流れです。
...
こうしてメンバーとタスクを一緒に取っておくと、画面側はメンバー数やタスクの進捗を追加の通信なしで表示できます。ただし、カードが使うのは件数だけなのに、タスクの行そのものを全部運んでいます。プロジェクトごとにタスクが数百件あれば、その数百行が毎回届きます。件数だけが欲しい場面では、Prisma の `_count` を使うとDB側で数えてから数字だけを返せます。今日は `include` の形を覚えることを優先しています。`orderBy: { createdAt: 'desc' }` は作成日の新しい順に並べる指定です。最後の `}),` で `getAll` を閉じ、`});` で `projectRouter` 全体を閉じます。
...
スクリーンショット: 下の画像は Step 9 まで書き終えた完成後の画面です。赤枠の中が、この Step で出る見出しです。いまの自分の画面には、右上のアーカイブ表示スイッチと新規プロジェクトボタン、そして下のカードがまだ出ていません。カードは Step 6、ヘッダーの2つは Step 9 で足します。
...
`api` は、サーバー側に置いたルーターへつながる入口です。ここから呼べる関数の名前も、渡せる引数の形も、サーバー側の定義から自動で決まります。だから `getAll` を打ち間違えると、ブラウザで開く前にエディタが赤い波線で教えてくれます。手書きの `fetch` では、URL を間違えても実行するまで気づけません。

`useState` は、アーカイブ表示を出すかどうかの選択を覚えておくために使います。ただの変数に入れると、画面が描き直されたときに値が初期状態へ戻ります。
...
`useQuery` に `isArchived: showArchived` を渡しています。スイッチを切り替えると `showArchived` の値が変わり、tRPC はそれを別の問い合わせとして扱って取り直します。取得しなおす関数を自分で呼ぶ必要はありません。切り替えのたびに再取得を書く形にすると、書き忘れた場所だけ古い一覧が残ります。

`projects` は、返事が届くまで `undefined` です。ここで空配列を初期値にすると、読み込み中と0件が同じ見た目になります。「まだ来ていない」と「1件も無い」は読者にとって別の意味なので、`isLoading` を別に受け取って分けて扱います。
...
スクリーンショット: 読み込んでいる間は、本文の場所にスピナーだけが出ます。

![読み込み中のプロジェクト一覧。左のサイドバーは出たままで、右の本文の場所に丸いスピナーだけが回っている](./screenshots/day09/project-loading.png)
...
`ProjectCard` は、色帯・メンバー数・進捗バーまで作り込んであるカード部品です。今日はこれを自分で書きません。Step 0 の `getAll` が返してきたデータを流し込むことに集中します。`TASK_STATUS` のほうは、そのデータに含まれる `status` と突き合わせるための定数です。完了を表す文字列が `TASK_STATUS.DONE` です。Step 6 で完了件数を数えるときに使います。この2行を取り込まないまま Step 6 へ進むと、どちらの名前も「見つかりません」という型エラーになります。
...
スクリーンショット: 下の画像は Step 9 まで書き終えた完成後の画面です。赤枠の中が、この Step で足したカードのグリッドです。いまの自分の画面には、右上のアーカイブ表示スイッチと新規プロジェクトボタンがまだ出ていません。どちらも Step 9 で足します。

![完成後のプロジェクト一覧。赤枠の中はカードのグリッドで、プロジェクトカードが1枚だけ並ぶ](./screenshots/day09/project-cards.png)

写っているカードは1枚です。初期データのうち読者がメンバーになっているのは「Webサイトリニューアル」だけなので、これで正しい状態です。


> `'DONE'` のような文字列リテラルではなく `TASK_STATUS.DONE` 定数を使います。定数を使うとタイプミスを防げて、値が変わっても1か所直すだけで済みます。
...
Step 6 で書いたグリッドの `<div>` の中身を、三項演算子（条件によって2つのうち片方を選ぶ書き方）を使う形に書き直します。**Step 6 で書いた `{projects?.map((project) => {` から `})}` までは、先にすべて削除してください。** 残したまま足すと、同じカードの記述が二重になります。
...
props の並びは Step 6 とまったく同じです。書き換えたのは前後の囲みだけです。末尾の `})` が map を閉じます。ただし `? (` で開いた三項演算子は、まだ閉じていません。ここで手を止めて保存すると、括弧の対応が合わずエラーになります。次のブロックの `) : (` から書き始めて、0件のときの表示までを続けて入れてください。
...
0件の画面を出すには、アカウントを入れ替えます。いま使っている `admin@example.com` は初期データの「Webサイトリニューアル」のメンバーなので、必ず1件返るためです。サイドバーからログアウトし、Day 07 の表にある `empty@example.com` / `password123` でログインし直すと、どのプロジェクトにも参加していない状態の画面が出ます。確かめたら `admin@example.com` に戻してください。以降の Step は管理者のまま進めます。
...
> いまはアーカイブ済みが1件も無いので、ON にすると一覧が空になります。これで正常です。
...
このコードも一覧は表示できます。困るのは、状態を自分で数え続けなければならない点です。`fetch` が失敗したときの分岐がどこにも無いので、通信が切れても `isLoading` は `false` になり、画面には空の一覧が出ます。読者には「0件でした」としか見えません。

`useEffect` の依存配列が空なので、取得は最初の1回だけです。今日のアーカイブ表示スイッチのように条件が増えた時点で、この形は作り直しになります。
...
`data: projects = []` と既定値を置いていますが、これは Step 2 で避けたのと同じ書き方です。使い分けの境目は、`isLoading` の分岐を先に書いたかどうかにあります。Step 2 では分岐より前だったので、空配列にすると読み込み中と0件が同じ見た目になりました。ここでは `isLoading` と `error` を先に `return` で片付けているため、この行に来た時点で両者はすでに分かれています。分岐を先に置いた場合だけ、既定値を使えます。

取得・読み込み中・失敗の3つが、1つのフックから揃って返ります。`isLoading` と `error` を先に分岐で片付けているので、その後ろでは `projects` の中身だけを考えれば済みます。`data: projects = []` と既定値を置いているのは、この位置から先で「配列である」と決め切るためです。分岐を通り抜けるたびに `undefined` かどうかを確かめる書き方は、確かめ忘れた1か所で画面が真っ白になります。
...
**Q1. `getAll` の `where.members = { some: { userId: ctx.session.userId } }` は、どのプロジェクトを選ぶ条件ですか。**

A. ログイン中の利用者がメンバーに含まれるプロジェクトだけを選びます。`some` は「関係する行のうち少なくとも1件が条件に合う」という意味です。メンバーの行を1件ずつ見て、自分の `userId` が見つかったプロジェクトだけが残ります。この条件が無いと、他人のプロジェクトまで一覧に並びます。

**Q2. `const { data: projects } = ...` を `const { data: projects = [] } = ...` に書き換えると、画面はどう変わりますか。**

A. 読み込みが終わる前の `projects` が、`undefined` ではなく空の配列になります。すると「まだ届いていない」と「0件だった」の2つが同じ見た目になります。`isLoading` の分岐より前では、この2つを区別できません。空状態の案内を先に出してしまう原因になります。

**Q3. 空状態を出す `<div>` に `col-span-full` を付けるのは、なぜですか。**

A. この `<div>` がグリッドの子だからです。付けないと、グリッドは他のカードと同じ1列分の幅を割り当てます。案内の文章が細い列に押し込まれ、左端に寄って表示されます。`col-span-full` は「全部の列にまたがれ」という指定で、これで横いっぱいに広がります。
...
完成版には `src/app/project/page.tsx` がありますが、そちらは Day 12 と Day 27 まで書き足した後の姿です。作成ダイアログ、削除確認、アーカイブ、詳細表示が入っているので、今日の終わりの手元のコードより長くなります。今日の時点では、一覧を取ってきて並べるところまでが書けていれば大丈夫です。足りない部分は、これから何日かかけて自分で足していきます。（販売用 ZIP に完成版の `src/` は入っていません。ここに書いた完成版との差は、この先どう伸びるかの説明として読んでください。合否は教材内の確認ポイントで判断します）。
