...
この時点の画面は、後で作るページヘッダー（新規プロジェクトボタンなど）がまだ無い状態で、カードだけがグリッドに並びます。完成形の見た目はこの日の最後のスクリーンショットで確認できます。

<!-- TODO(screenshot): day09-cards.png は day09-complete.png と同一ファイルだったため参照を外しました。ヘッダー実装前の途中状態（カードのみのグリッド）を実キャプチャして挿入します -->
