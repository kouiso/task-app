...
| `PageLoadingSpinner` | スピナー付きの全画面ローディング表示 | `AppLayout` を内蔵している |

> ⚠️ **重要**: `PageLoadingSpinner` は内部で `AppLayout`（サイドバー付きレイアウト）を含んでいます。そのため `<AppLayout>` の中に `<PageLoadingSpinner />` を入れると、サイドバーが二重に表示されてしまいます。
...
> 💡 `Suspense` は子コンポーネント（`ProjectPageContent`）が準備完了するまで、代わりに `fallback` に指定した `PageLoadingSpinner` を表示します。`PageLoadingSpinner` は内部にサイドバー付きレイアウトを含んでいるので、ローディング中もアプリの見た目が崩れません。
...
> ⚠️ **注意**: ここで `<AppLayout><PageLoadingSpinner /></AppLayout>` と書くと、サイドバーが二重に表示されてしまいます。`PageLoadingSpinner` は内部に `AppLayout` を含んでいるので、そのまま `<PageLoadingSpinner />` だけで正しいレイアウトが表示されます。
...
| サイドバーが二重に表示される | `PageLoadingSpinner` を `AppLayout` で囲んでいる | `PageLoadingSpinner` は単体で使う（内部にAppLayout含む） |
