...
完成版には `src/app/project/page.tsx` がありますが、そちらは Day 12 と Day 27 まで書き足した後の姿です。作成ダイアログ、削除確認、アーカイブ、詳細表示が入っているので、今日の終わりの手元のコードより長くなります。今日の時点では、一覧を取ってきて並べるところまでが書けていれば大丈夫です。足りない部分は、これから何日かかけて自分で足していきます。（販売用 ZIP に完成版の `src/` は入っていません。上に挙げた完成版との差は、この先どう伸びるかの説明として読んでください。合否は教材内の確認ポイントで判断します）。
