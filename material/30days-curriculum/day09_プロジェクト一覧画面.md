...
このリポジトリにも `src/app/project/page.tsx` がありますが、そちらは Day 12 と Day 27 まで書き足した後の姿です。作成ダイアログ、削除確認、アーカイブ、詳細表示が入っているので、今日の終わりの手元のコードより長くなります。今日の時点で見比べるなら、一覧を取ってきて並べるところまでが一致していれば大丈夫です。足りない部分は、これから何日かかけて自分で足していきます。
