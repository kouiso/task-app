...
> **今日のゴールライン**: 3画面あって量は多いけど、14ステップに分かれていて1つ3-5分。プロフィール表示 → 編集 → パスワード変更を順番に追いかけるだけで完成します。全部一気に理解しようとしなくてよい。
...
プロフィール表示は動きますが、`currentUser` の null チェックが繰り返されて、表示したい項目が埋もれています。プロの現場では `?.` と `??` でアクセスの形をそろえます。
