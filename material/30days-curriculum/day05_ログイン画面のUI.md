...
`.email()` や `.min(1)` に渡している日本語は、失敗したときに画面へ出す文言そのものです。ルールと文言が同じ行に並んでいるので、「どの条件で何と表示されるか」を1か所で読み切れます。ここを空文字にすると英語の既定メッセージが出てしまうため、必ず日本語を渡します。
...
`import` の並び順は気にしなくて構いません。保存すると Biome（このプロジェクトが使っている整形ツール）が自動で並べ替えます。
...
パスワード欄も、やっていることはメール欄と変わりません。`register('password')` の文字列が Step 2 で決めた項目名と一致しているから、入力した値が正しい場所に入ります。この文字列が `passwrod` のように1文字でも違うと、画面は普通に打てるのに送信データがいつも空になります。
...
`type="submit"` が付いたボタンは、押されるとフォームの `onSubmit`、つまり Step 4 で書いた `handleSubmit(onSubmit)` を呼びます。ボタン自身に処理を書いていないのに反応するのは、この仕組みのおかげです。ここまでで送信の流れは動きますが、送信先はまだ `console.log` のままです。ブラウザの開発者ツール（F12 キーで開く画面）のコンソールに入力値が出れば、その先が未完成でも今日のここまでは合格です。
...
入力欄の中身は Step 4 で書いたものと1文字も変わりません。変わったのは、`<form>` が `CardContent` の内側へ移った点だけです。`register` と `errors` はどちらも同じ `useForm` から来ているので、囲む箱を替えてもフォームの動きは変わりません。見た目の階層と、値を管理する仕組みが独立している証拠です。
...
パスワード欄も Step 5 の内容をそのまま持ってきています。2つの `<div>` が並んだだけの形ですが、外側の `<form>` に付けた `className="space-y-4"` が、この2つのあいだに自動で縦の余白を入れます。欄ごとに `margin` を書かなくても間隔が揃うのは、親がまとめて面倒を見ているからです。
...
ここで `return (` から始まった長い式がようやく閉じます。閉じタグの並びは、開いた順のちょうど逆です。`<form>` の中で最後に開いた `<Button>` を閉じ、続けて `</form>`、`</CardContent>`、`</Card>`、外側の `</div>`、そして `);` と、内側から外側へ戻っていきます。順番を入れ替えると、保存した瞬間にエラーが出ます。迷ったら開いた順を上から読み返してください。

保存すると、Step 6 の途中から出ていた赤線が消えます。消えない場合は、閉じタグの数が足りないか多いかのどちらかです。エディタでどれかのタグをクリックすると対になる相手が光るので、光らないタグが犯人です。

画面には枠線と影の付いたカードが中央に出ます。中の入力欄は Step 5 までと同じ動きのままです。空欄で送信すれば赤いメッセージが出て、埋めればコンソールに値が出ます。見た目を整えただけで、入力の扱いには一切手を入れていません。
...
これは zod を知らずにフォーム検査を書いた場合の、ごく普通のコードです。`isEmail` の正規表現も `password.length < 8` の条件も、やっていることは Step 2 の `loginSchema` と変わりません。読みにくくもないはずです。
