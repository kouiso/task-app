...
> `'use client'` は、この部品をブラウザ側でも動かすための宣言です。今回の `useForm` やイベント処理に必要です。ページ全体へ付ける以外に、操作する部品だけを別ファイルへ分けて付ける書き方もあります。
...
`type="submit"` が付いたボタンは、押されるとフォームの `onSubmit`、つまり Step 4 で書いた `handleSubmit(onSubmit)` を呼びます。ボタン自身に処理を書いていないのに反応するのは、この仕組みのおかげです。ここまでで送信の流れは動きますが、送信先はまだ `console.log` のままです。ブラウザの開発者ツール（macOS では `option + command + I`、Windows では `F12` で開く画面）のコンソールに「入力チェックが通りました」と出れば、フォームの入力検査と送信処理はつながっています。入力値やパスワードはログへ出しません。
...
`onSuccess` で `window.location.replace(callbackUrl)` を使っているのには理由があります。Next.js の `router.push` は、ブラウザが保持している画面の一部や状態を使いながら移動します。ログイン前の表示や取得済みデータが残らないよう、この教材ではページ全体を読み込み直します。`window.location.replace` なら、ログイン後の Cookie を使ってページを取得し、画面の状態も作り直せます。`replace` は履歴を置き換える指示でもあるので、遷移後に戻るボタンを押してもログイン画面には戻りません。
...
Step 3 の確認メッセージを、サーバーへ送信する処理に置き換えました。送信する `data` にはパスワードが含まれるので、確認のためでもログへ出しません。
...
- 同じメールアドレスを同じ回線から5回続けて失敗すると15分ログインできなくなるので、試すのは2〜3回にとどめる
...
`Suspense` は React 側の部品で、中身の準備が終わるまでのあいだ、代わりの表示を出しておく仕組みです。開発中は `Suspense` がなくても表示されることがあります。ただし、この静的ページを本番用にビルドすると `useSearchParams` を包む `Suspense` が必要になるため、ここで追加します。
...
成功と失敗の行き先を、送信の手前で1回だけ決めている部分です。`onSuccess` と `onError` をここに書いておけば、送信のたびに同じ分岐を書き直す必要がありません。`window.location.replace` はページ全体を読み込み直します。ログイン前の表示や取得済みデータを持ち越さず、ログイン後の Cookie を使ってページを取得するために使っています。
...
Step 3 の確認メッセージを、サーバーへの送信処理に置き換えています。パスワードを含む `data` はログへ出しません。`setError(null)` を先に呼んでいるのは、前回の失敗で出した赤い帯を消すためです。この1行が無いと、2回目の送信が成功しても古いエラーが画面に残り続けます。
...
- [ ] `{...register('email')}` と `{...register('password')}` で入力欄を登録できた
...
A. ログイン前の表示や取得済みデータを持ち越さないためです。`router.push` はブラウザが保持している画面の一部や状態を使って移動します。この教材では `window.location.replace` でページ全体を読み込み直し、ログイン後の Cookie でページを取得します。履歴も置き換わります。そのため遷移したあとに戻るボタンを押しても、ログイン画面へは戻りません。
