...
`edu-creator/`は、CrewAIを使った教材自動生成ツール。
実装AIが内部のプロンプト・コードを編集する際の心構えを以下に記す。

### 基本概念：実装AI vs eduAI

| 名称 | 役割 |
|------|------|
| **実装AI** | 内田祐貴が使うAI。edu-creatorのコード・プロンプトを編集 |
| **eduAI** | CrewAI内のエージェント（Scout, Scribe等）。教材を生成する |
...
| 最終出力（チュートリアル） | **日本語（です・ます調）** | 読者向け |

### ファイル編集時の注意

| ファイル | 影響範囲 | 注意点 |
|---------|---------|--------|
| `prompt/task/quality_rules.py` | **全生成物** | 変更前に影響を検討 |
| `prompt/task/write.py` | Scribeの出力 | 見出しルール厳密遵守 |
| `config_models.py` | 設定検証 | 設定追加時は必ず更新 |
...
1. **マイクロマネジメント** - 一挙一動を制御するな
2. **矛盾するルール** - ルールが多すぎて矛盾が生じるな
3. **「なぜ」を伝えない** - 「何を」だけで「なぜ」を伝えるな
4. **創造的な部分を固定** - AIの創造性を奪うな

**詳細は `.github/instructions/edu-creator.instructions.md` を参照のこと。**
