...
description: task-appで.ts/.tsxファイルを書く・直す時のTypeScript型安全性規約。any/as/typeof禁止パターンと正しい代替、Props型命名規約。
---

# TypeScript規約

原本: `prompt/instructions/typescript.instructions.md`（Copilot向け、変更していない）。
...
**「`as`を書く→指摘→修正→また`as`を書く」のループを二度と繰り返すな。**
