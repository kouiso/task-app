...
description: task-appでPRレビューやコードレビューを行う時の規約。レビュー観点、コメント優先度、bot指摘への自動resolveプロトコル。以前は「常時読み込み」と宣言されていたが実際には一度も読み込まれていなかったバグの修正版。
---

# コードレビュー規約 (Code Review Guidelines)

原本: `prompt/instructions/code-review.instructions.md`（Copilot向け、変更していない）。
対象: `**/*.ts,**/*.tsx,**/*.js,**/*.jsx,**/*.py,**/*.dart` のレビュー時。
