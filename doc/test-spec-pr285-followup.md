...
## 自動テストによる確認（2026-07-27 追記）

上の「未実施項目まとめ」で高優先としていた2件は、API層の自動テストがすでに存在していました。
テスト用DBを立てて実行し、通ることを確認しています。

```
npx vitest run src/server/api/routers/__test/
...
条件は仕様書の想定と一致しますが、経路は違います。仕様書は本番環境で画面から操作する手順を
想定しており、ここで通したのはテスト用DBに対する呼び出しです。画面経由の確認は残ります。

TC-03・TC-04・TC-05 は画面の見え方を測る項目なので、自動テストでは代替できません。
