...
**裏を取れていない日は撮らせない。** `MAX_SEEDED_DAY` より先の日は、指定しても止まる。
足りないデータで撮った画像は、完成版で撮った画像と同じ種類の嘘になる。
Day 18–30 の作業でここを **30** まで伸ばした（下の「Day 18–30 で撮ったもの」を見る）。
...
切り抜きが小さすぎると紙面で粗くなる。版面の幅は 152.4mm（6.0inch）で、`figure img` は
そこいっぱいに置かれるので、**画像の幅が 720px を切ると実効解像度が 120ppi の下限を割る**。
`[role='tablist']` だけを切り出した回が 700px で落ちた。狭い部品は切り抜かず、画面全体を
撮って赤枠で指す。
...
9. **`<input type="date">` の書式が英語圏の並び（`mm/dd/yyyy`）で出た。** あの欄が見ているのは
   context の `locale` でも `--lang` でもなく、プロセスのロケールだった。`LANG` と `LC_ALL` を
   渡すと `yyyy/mm/dd` になる。日本語版のブラウザが出す `年/月/日` までは届かない
   （このコンテナに Chromium の日本語リソースが無い）。並びは読者の画面と同じになり、残る差は
   文字だけなので、そのことを `day10` と `day14` の本文へ1行入れた。
...
| `day17/status-tabs.png` | 17 | 同じ画像の2回目。Step 4 が言う Tabs を赤枠で指す |
...
`--all` は日をワーカープールで回す（同時 3 まで）。ワーカーごとに DB 名とポート帯を分けてある
ので、隣の日のデータを見て撮ることはない。詳しくは下の「並列で撮る」を見る。
...
Day 18 以降を撮るために、道具へ4つ足した。どれも「その日の画面」を撮るために要ったもので、
見栄えのために足したものは1つも無い。

### 1. 読み込み中を撮る（`stall`）

手元の DB は速すぎて、スピナーの出ている一瞬を狙えない。宣言表に `stall` を書くと、
名指しした通信（`report.getOverview` 等）だけを 20 秒返さずに待たせる。返事を捨てずに
待たせるのは、撮りたいのが失敗の表示ではなく「まだ返ってきていない」表示だから。
止めた通信があると `load` は返ってこないので、その回だけ `commit` で先へ進める。

**入口に気をつける。** tRPC は近い時刻の呼び出しを1本の通信へまとめる。`/project` を直に開くと
`auth.getSession` が `project.getAll` と同じ通信に乗り、止めた側に引きずられて `AppLayout`
ごと出てこない。ダッシュボードから入ってサイドバーのリンクを押すと、セッションは取得済みなので
止まるのは一覧の取得だけになる。読者がサイドバーから移るのと同じ動きでもある。

**読み込み中の1枚は切り抜かない。** スピナーの入れ物だけを切り出すと、どのページで撮っても
同じ絵になる。実際に `day09/project-loading.png` と `day29/user-detail-loading.png` が
バイト単位で一致し、CI の重複検査が落ちた。紙面の9割が白場にもなる。いまは切り抜かず、
**その日の完成画面と同じ高さ**でページの枠ごと撮る。

| 出力 | 窓の高さ | 画面に枠があるか |
|---|---|---|
| `day09/project-loading.png` | 420（`day09/project-cards.png` と同じ） | あり（`AppLayout` の内側で返す） |
| `day21/report-loading.png` | 434（`day21/report.png` と同じ） | **なし**（`AppLayout` の外で返す） |
| `day23/report-weekly-loading.png` | 640 | **なし**（同上。完成画面は 1022 だが、枠が無いので伸ばしても白場が増えるだけ） |
| `day29/user-detail-loading.png` | 610（`day29/user-detail.png` と同じ） | あり |

枠のある日はサイドバーと選択中のメニューが写るので、読者はどのページの読み込みかを判定できる。
枠の無い日（Day 21・23）は白い画面とスピナーだけになるが、それが読者の画面そのもので、
本文も「サイドバーも消えます」と書いている。

### 2. 窓の高さを中身へ合わせる（`fitToContent`）

アプリの外枠は `h-screen` なので、窓の高さがそのまま画像の高さになる。中身の短い日は下半分が
白場になり、長い日は `main` の中でスクロールして下が切れる（`fullPage` は文書のスクロールしか
追わない）。`full_page` の回だけ、撮る直前に窓の高さを中身へ合わせる。

測るのは `main` の**子要素のいちばん下 + `padding-bottom`**。`main.scrollHeight` は使えない。
`h-screen` の中の `flex-1` なので、中身が短くても窓より小さい値を返さず、縮める判断ができない。

幅は 1440 のまま動かさない。直すのは縦だけ。

### 3. `next dev` で出す日（Day 11）

Day 11 のツリーは `npm run build` が通らない。教材自身が「Day 11 を終えた時点で
`npm run build` は通りません」と断っており、`build_day_snapshots.py` の `EXPECTED_RED` も
そう扱っている。その日の本文は `npm run dev` で確認させるので、撮る側も `next dev` で出す。
Next.js の開発用の目印（`nextjs-portal`）は、読者が書いた画面ではないので隠してから撮る。

### 4. 並列で撮る

詰まりは DB だけだった。ワーカーごとに DB 名を変え（`shoot_w0` / `shoot_w1` / `shoot_w2`）、
ポートも帯で分ける。同時実行は 3 まで（4コアの機械で、1日ぶんが `next start` 1本と
Chromium 1本を使うため）。日のループを `ThreadPoolExecutor` で回し、出力は日ごとにまとめて
出す（行が混ざると、どの日で落ちたか読めない）。

`test_shoot_screenshots.py` に、ワーカーが別々の DB を見ていることを確かめる検査を足した。

---

## 撮ってみて分かった欠陥（Day 18–30 の担当へ）

1. **`day16:916` の `task-list.png` はまだ撮れない。** `dist/day-snapshots/day16/src/app/task/page.tsx`
   が Day 15 の版で止まり、`<TaskCard>` へ `timeSpentMinutes` が渡らない。Day 16 の完成コードに
   このファイルの全文が無く、Step 3 の断片（`handleTimeLogSuccess` という新しい宣言）に
   ツリー組み立てが当てられる足場が無いのが原因。同じ形で Day 28 の一括操作の UI も組めない。
   直すには、教材側にそのファイルの全文を1回載せる必要がある。
2. **`day09:441` と `day29:577` の alt が実物と食い違っていた。** どちらも「サイドバーも消え」
   「スピナーだけ」と書いていたが、この2日の読み込み中は `AppLayout` の**内側**で返すので
   サイドバーは出たままだった。切り抜いて撮っていた間は誰も気づけなかった。撮り方を変えた回に
   本文の側も直した。
3. **`day11:698` の一覧は、編集を実行せずに撮ってあった。** Day 10 の作成直後と1バイトも
   同じ画像になっていた。本文は「編集後に更新された一覧」と言うので、Step 10 の編集フロー
   （名前を変えて「更新」）を実際に通してから撮り直した。変更後の名前は本文が指定していないので
   `ポートフォリオ（改）` を選び、その旨をキャプションに書いた。カードの見出しは幅で切り詰められる
   ので、長い名前にすると画像の中で欠ける。
4. **`day30` の「本番環境のログイン画面」は手元の画面だった。** デプロイ先は Vercel の SSO で
   保護されていて外から開けない。画面そのものは手元と同じなので、本文を「Day 05 で作った
   ログイン画面を手元で撮ったもの」と言い直し、重複していた `day30/login.png` は消した。

**同じ形を見つける手順**: 撮り終わったら全数を md5 で見る。別の日の別の状態として撮ったはずの
2枚が一致していたら、たいてい「その日の操作を実行せずに撮っている」印である。

```bash
find material/30days-curriculum/screenshots -name '*.png' | xargs md5sum | sort | \
  awk '{c[$1]=c[$1]" "$2; n[$1]++} END {for(k in c) if(n[k]>1) print n[k], k, c[k]}'
```
