...
実測は7件それぞれで取った。更新時刻を進めると修正後は古さを検知し、修正前の
数え方では素通りする。材料は `main` の78件から101件になった（`tree_inputs(3)` の実測。
増えた分の内訳はパッチ1件と、見張るディレクトリ16件。作業途中で数えた「79」は
最初の1件を足したあとの値で、変更前の値やない）。
