...
件数が古い。**七巡目は CodeRabbit が6件 — DB マーカーだけで SKIP にすると本物の失敗が exit 0 で出ていく（Codex と逆向きの穴。両側判定へ変更）／画面の日別行が切り分け前の状態／環境変数の判定が拒否リストのままで綴り間違いが WARNING へ落ちる／`duplicate-image-gate.md` の doc 3件。**合計19件、7ラウンド。**

判定と直し方は `doc/review-handoff/coderabbit-verdicts.md` の末尾。**19件とも退行テストを足し、
...
PR #389 のボット指摘は19件とも直して返信・resolve 済み。マージは局長の指示待ち。
