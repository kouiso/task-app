...
最終更新: 2026-08-31 / ブランチ `claude/30days-material-product-ready-hor18z` / PR **#388**（GitHub上ではMERGED）

> 現行監査メモ（2026-08-31 確認時点）: ローカル HEAD は `7ce2895`、GitHub の同名ブランチは `1887a23`。
> ローカルに未コミット変更があり、履歴も分岐しているため、pull・push・PR操作は保留中。
> pull は競合で abort した。最新のリモートはfetch済みで、ローカルの作業中変更は保持している。
> PR のマージはこの作業から行っていない。
...
| ③ | **写経したのに動かん** | ✅ | 既存の通し実測は **29/30**。今回変更した day04・07・12・27・30 も個別の `--verify` で tsc / build とも OK。day11 は教材が「ここは赤くなります」と断っとる想定どおり |
| ④ | 誤字・矛盾 | ✅ | 70件＋走査95件の裏取りで29件 |
| ⑤ | 紙面が崩れる | ✅ | `check_pdf_book.py` / `check_page_layout.py` とも **36冊 exit 0** |
| ⑥ | スクショと自分の画面が違う | ✅ | 現行の画像111枚を確認。`check_visualization.py --fail-on-duplicate-image` は34ファイルすべて FAIL 0 |
| ⑦ | コードだけ並んで説明が無い | ✅ | 現行の Mermaid は **71ブロック**。day01〜30の各教材に図がある |
| ⑧ | 30日終わって理解が残らん | ✅ | 理解チェック 3問×30日 = 90問 |
| ⑨ | Step ごとに撮ってへん（同じ画像の使い回し） | ✅ | 同一日の画像重複は **0件**。残る同一パス8件は、別日の導入・完成版・付録からの意図した参照 |
| ⑩ | 写真に「まだ作ってへんもの」が映る | ✅ | 追加撮影分を含む現行111枚にバイト重複なし。未参照画像も **0枚** |

**現行ツリーの機械ゲート上、①〜⑩に出荷ブロッカーはありません。**
ただし、未コミット変更の扱いと、GitHub 側の同名ブランチ／マージ済み PR との統合方針は未決定です。

### 2026-08-31 現行監査の実測

- `npx textlint "material/**/*.md"`: `ok`
- `check_quality.sh material/30days-curriculum/`: `ALL CHECKS PASS`
- 画像: **111枚 / 125参照 / 同一日重複0件 / 未参照0枚**
- Mermaid: **71ブロック**
- 販売 ZIP: **74ファイル / 88KB**。`test_sale_package.py`: **31/31**
- PDF: **36冊**。商品検査・紙面検査とも **36/36**。検査テストは **44 / 72** ケース
- 日別再構成: 今回変更した day04・07・12・27・30 は `tsc OK / build OK`
- `npm run test -- --reporter=dot`: `.claude/worktrees` を除外後、**23ファイル・233テスト PASS**
- `build_day_snapshots.py --all --verify`: **30日完走 / tsc・build とも29日 OK**。day11 は本文が明示した想定内の赤。結果は `day-snapshots-result.md` に記録

---

## 旧記録 — 残作業①（現行ツリーでは再監査済み）
