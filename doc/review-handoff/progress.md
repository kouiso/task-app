...
| ③ | **写経したのに動かん** | ✅ | CI で30日ぶん全部を判定し、判定を飛ばした日は0件（run 33460433956）。day11 は教材が「ここは赤くなります」と断っとる想定どおりの赤 |
...
実測は5件それぞれで取った。更新時刻を進めると修正後は古さを検知し、修正前の
数え方では素通りする。材料は `main` の78件から83件になった（`tree_inputs(3)` の実測。
