...
`tree_inputs()` が `tsconfig.json` を材料に数えてへんかった。この設定は複写やのうて
`write_reader_tsconfig` が読んで書き直す入力なので、借り物の一覧に載らず、材料からも
落ちとった。型検査の設定を変えた回はツリーが古いまま撮れて、画像だけが前のアプリの
まま残る。**撮れてしまうから誰も気づけん**類の穴で、⑥と同じ形や。

材料に足して、自己テストへ「読者の型設定を作る元を見ているか」を1件加えた。
mtime を触って実測すると、修正前の数え方では材料の最新時刻が動かず、修正後は動く。
