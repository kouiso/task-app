...
最終更新: 2026-08-31 / **#388 と #389 はどちらもマージ済み**（#389 = `be886bcc` / 17:35:57Z）。
作業ブランチ `claude/30days-material-product-ready-hor18z` はマージ後に GitHub 側で削除されとる。
**続きを書くときは `origin/main` から新しくブランチを切る。**消えたブランチに積んでも push でけへん。

10類型は全部閉じた。ここから先は「出荷できる状態」から「実際に配る」へ移る段で、
タグもリリースも0件なので**まだ1つも配られてへん**。配布の段取りは局長の指示待ち。
...
2026-08-31、局長の指示を受けて #388 の本文へ追記した。
**ここに前まで書いてあった「#388 の本文は 6.2M / 174ファイルで止まっとる」は誤りやった。**
本文を取り直して確かめたら、ZIP の数字は最初から `88,757 byte / 74ファイル` で正しかった。
実際に古かったのは別の場所で、⑥⑦⑨⑩ を「残す4類型」と書いた節が、
その後このPRの中で全部潰したことを反映してへんかった。そこへ日付つきの追記を足しとる。

教訓は前と同じや。**記録の古さを別の記録から孫引きせん。**現物を取って確かめる。
...
**#389 は 2026-08-31 17:35:57Z に squash merge 済み**（`be886bcc`）。62本のレビュースレッドは
未解決0件、CI は25件すべて緑で、`教材PDF検査` はそのとき `scope=all` で36冊全部を組んどる。

---

## 写経ビルドの見張りを CI に載せた（2026-08-31・マージ後の追い）

### なぜ足したか

⑤紙面と①ZIPには CI ゲートがあるのに、**③「写経したのに動かん」だけ機械の見張りが無かった**。
`check_quality.sh` に載っとったのは `test_build_day_snapshots`（道具の自己テスト）で、
**道具を走らせた結果**は誰も見てへんかった。教材のコードブロックを1行直しても、
`--all --verify` を人が手で回さん限り気づけん状態やった。

不満10類型で③は「写経したのに動かん」。買った人が最初に当たる壁で、
ここが壊れたまま出ると次は買ってもらえん。手で回す運用に預けるのは弱すぎる。

### 置き場と起動条件

| ファイル | 役割 |
|---|---|
| `.github/workflows/snapshot-gate.yml` | 教材md・scaffold・組み立て側が動いた PR で `--all --verify` を回す。週次と手動起動も持つ |

`pdf-book-full.yml` と同じ考え方で、**差分に現れんドリフト**（`npm` の推移的依存・runner の
Node 版）を週次で拾う。PR 側は教材と組み立てに触った回だけ回す。

### 判定の線

`--all --verify` は day11 が想定内で赤い（教材が本文で「ここは赤くなります」と断っとる）。
だから**「30日全部緑」を条件にしたらゲートが永久に赤い**。条件は
**「想定内として名指しした日以外が全部緑」**。名指しは道具側の `EXPECTED_RED` が持っとる。

### 載せた直後は飾りやった（2026-09-01・#390）

ゲートを置いた時点では、**教材だけ触った PR で1日も検証してへんかった**。
差分の一覧は既定でパスを8進エスケープする。教材は30日ぶん全部が日本語名なので、
`^material/` の判定が丸ごと空振りして `run=false` になる。空振りを拾うはずの
トリップワイヤも同じ理由で当たらん。#390 の親コミットで測ると、変更された
パスは32件すべてクォート付きで返っとった。紙面側は先に同じ穴を踏んで
`-z` と `core.quotePath=false` を使っとる。写経側だけ揃ってへんかった。

同じ回で塞いだ穴があと4つ。

| 穴 | 何が起きるか |
|---|---|
| `tsconfig.json` / `scripts/build-zip.sh` / `.node-version` が対象一覧に無い | 現物を読んどるのに「対象なし」で緑 |
| 日付の点呼が無い | day を1本消すと29日で通り、30日ゲートが29日ゲートに化ける |
| 結果表がリポジトリに入っとる | 手前で落ちた回に、前回の表が成果物として上がる |
| ループ変数が未使用 | actionlint（ShellCheck SC2034）の指摘 |

Codex の「DB のテーブルを作らんと落ちる」は反証。空のデータベースのまま30日ぶん
全部が build まで通り、判定を飛ばした日は0件やった（run 33456840786 と 33460433956）。

**#390 は 2026-09-01 02:14:53Z に squash merge 済み**（`b12ef340`）。ボットのレビュー
スレッドは6本とも返信して close、未解決0。マージ前のチェックは pass 22 / skipping 3 /
fail 0。追加した3工程はいずれも success で、「day01〜day30 の30日ぶん、全部が
判定されとる」「SKIP なし」を出力しとる。

## 撮影の古さ判定にも同じ形の穴があった（2026-09-01・#390 の追い）

`tree_inputs()` が「読まれるだけで複写されん入力」を3件とも数えてへんかった。
借り物の一覧（`BORROWED_FILES`）に載せると中身がそのまま置かれてまうので載せられん。
載せられんぶん材料からも落ちる、という形の穴や。

| 入力 | 誰が読むか | 落ちると何が起きるか |
|---|---|---|
| `tsconfig.json` | `write_reader_tsconfig` | 型検査の設定を変えた回に古い画面のまま撮れる |
| `scripts/scaffold-from-scratch.sh` | `scaffold_tsconfig_excludes` | 読者の tsconfig の exclude を変えても気づかん |
| `scripts/build-zip.sh` | `sale_package` | 配布物の一覧を変えてもツリーが組み直されん |

撮影は成功してしまうので、**撮れてしまうから誰も気づけん**。⑥と同じ形や。
3件を `READ_ONLY_INPUTS` にまとめて材料に数えた。

### 手で並べる限り抜ける、と分かった

同じ穴が1段奥にもあった。`curriculum_blocks` はフェンスの解釈を `markdown_scan` から、
`sale_package` は配布物の対応表を `check_scaffold_curriculum_alignment` から取っとる。
どちらが動いてもツリーの中身は変わるのに、直接 import した分だけ並べた一覧からは落ちる。
Codex が2回に分けて指摘してきた形が全部これで、**一覧を手で維持する方式そのものが原因**や。

だから検査側を変えた。`BUILDER_SOURCES` から import を辿って、同じディレクトリの
`.py` が一覧に無ければ落とす。一覧から2件抜くと3件の指摘が出ることを確かめとる。
次に誰かが import を足しても、一覧へ入れんかぎり自己テストが落ちる。

同じ一覧が CI 側にもある。材料に足しても `snapshot-gate.yml` の対象一覧に足し忘れると、
そのファイルだけ動いた PR は対象なしで素通りする。**一覧が2箇所にあること自体が原因**なので、
自己テストがワークフローの正規表現を読んで材料と突き合わせる。一覧から1件抜くと落ちる。

ロックファイル（`package-lock.json`）も材料に入れた。組んだツリーは `node_modules` を
このリポジトリから symlink で借りるので、ロックだけ動いた回はツリーの中身が1バイトも
変わらん。それでも中身の違う依存で build された `.next` が残るため、古い bundle のまま撮れる。
ツリーの中身を決める入力とは別物なので、`ENVIRONMENT_INPUTS` として分けとる。

ゲート側だけが持つ対象（`.node-version`・`Makefile`・ワークフロー自身）は材料に現れん。
材料から期待値を作るだけやと、その行を YAML から消しても緑のままになる。テスト側に
固定集合を置いて別に当てとる。

パッチも入れた。`postinstall` の patch-package が `node_modules` を書き換えるので、
パッチを1行直してもロックファイルは動かん。ロックだけ見とったら古い bundle のまま撮れる。

### 削除は mtime に現れん、という穴もあった

一番新しい材料の時刻で古さを測っとるが、**消えたファイルには mtime が無い**。教材や
配布物を1本消した回は時刻が動かず、消したはずの中身が残ったツリーを再利用する。
ディレクトリの mtime は中の増減で動くので、教材・パッチ・配布物の置き場そのものを
材料に足した。一時ファイルを作って消して実測すると、どちらでもディレクトリの時刻が動く。

置き場の一覧は「いま複写しとるファイル」やのうて `scripts/_*` の並びから作る。中身から
作ると、最後の1本を消した回にその置き場ごと監視から外れる。置き場そのものが消えた回は
親の並びが動くので、`scripts/` と `material/` も見とる。Node の版（`.node-version`）も
同じ理屈で足した。版を切り替えたら `.next` は組み直しが要る。

実測は8件それぞれで取った。更新時刻を進めると修正後は古さを検知し、修正前の
数え方では素通りする。置き場を空にしても監視に残ることも、中身を退避して確かめた。
配布物の置き場は入れ子も1つずつ見る。深い階層でファイルを消したとき、動くのはその
階層の時刻だけで、上は変わらんからや。`scripts` は無条件に入れる。いまある置き場から
作ると、最後の1つを消したコミットで期待値ごと消える。

土台の入力は4つ。npm の設定（`.npmrc`）、Node の版（`.node-version`）、mise の版
（`.mise.toml`）、ロックファイル。どれも中身を変えると `npm ci` の結果が変わるのに、
ツリーの中身は1バイトも動かん。置くかどうかが任意のものもあるので、無い物は材料から
落とす。落ちても、その削除は置き場の時刻が拾う。

改名は差分に行き先だけが出る。`--no-renames` を付けた時だけ元のパスも出た（手元で
R100 の改名を作って確認）。ゲートの差分取得に付けとる。

材料は `main` の78件から110件になった（`tree_inputs(3)` の実測。内訳はファイル88件と
ディレクトリ22件。作業途中で数えた「79」は最初の1件を足したあとの値で、変更前の値やない）。

### ここで打ち切った理由

Codex が9ラウンドで15件出してきて、どれも本物やった。ただ最後の数ラウンドは
「mtime で古さを測る」という元からの仕組みの隙を、順に潰していく形になっとった。
