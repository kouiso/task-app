...
`build_day_snapshots.py:1344` **Day 11 の診断が、許可コードの集合だけで別の欠陥を通しとった**（P1・採用）

- 根拠: `tsc_failure_is_expected()` は5件・`project-detail-view.tsx`・`getById` 1件・許可コード（`TS2339` / `TS7006` / `TS7053`）だけを見とった。そのため、同じファイルで `TS7006` / `TS7053` の本文が別の implicit-any / indexing 診断へ差し替わっても、件数とコードが残れば `broken_days()` が空になる。
- 直し: Day 11 の正本へ、コードとメッセージ接頭辞の組み合わせを重複込みで登録した。行番号とメッセージ後半の型情報は版で変わりうるため比較せず、診断の種類と根拠になる本文だけを順不同で照合する。`next build` 側も包み紙を除いた実エラーを1件に固定し、`getById` 以外の型エラーが同居したら免除しない。
- 回帰テスト: 断り書きどおりの `TS2339×1 / TS7006×2 / TS7053×2` は免除し、未知コード・別ファイル・別メッセージ・追加 build エラーは免除しない。別メッセージへの差し替えを戻すと自己テストが落ちることも確認した。
- 実測: `python3 scripts/curriculum-qa/test_build_day_snapshots.py` は **32/32 合格**。
