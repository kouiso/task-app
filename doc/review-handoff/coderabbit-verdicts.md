...
## [PR #389 二十四巡目] Codex 1件（採用）

`build_day_snapshots.py:1344` **入れ替わった診断を「想定内」で通しとった**（P1・採用）

- 根拠: `tsc_failure_is_expected()` が見とったのは件数（5）・場所（1ファイル）・**コードの所属**（許された3種のどれか）・識別子（`getById`）だけ。断り書きの `TS7006` が1件消えて、同じファイルの別の行に無関係な `TS7006` が1件入ると、**4つとも揃うたまま**別の欠陥が想定内で通り、`broken_days()` が空になって `--verify` が exit 0 になる。十九巡目で「コードまで名指しした」のは所属の話で、**どの診断か**は縛れてへんかった。
- 直し: 断り書きを**位置まで名指しした多重集合**にした。day11 のツリーで `npx tsc --noEmit` を実際に流して採った5件を `EXPECTED_RED_SIGNATURE[11]["diagnostics"]` に置く（実測 2026-08-31）:

  ```
  project-detail-view.tsx(29,47):TS2339 / (144,44):TS7006 / (167,31):TS7053
  project-detail-view.tsx(237,43):TS7006 / (246,26):TS7053
  ```

  `DIAGNOSTIC_HEAD` で各行を「位置＋コード」へ縮め、**丸ごと一致した時だけ**免除する。メッセージの尻尾は型の中身を吐いて長さで切られるので使わん。位置なら切られん上に、1件でも入れ替われば必ず変わる。
- 副産物: テスト側の day11 fixture が3箇所とも手書きで位置を並べとった。断り書きを更新した時にそこだけ古いまま緑になるので、`documented_day11_errors()` を1本作って**断り書きの現物から組む**形へ寄せた。
- 回帰テスト `check_expected_red_rejects_swapped_diagnostic`。断り書きの `(144,44)` を落として `(311,12)` の同じコードを足す — 件数・場所・コード・識別子は全部揃うたまま。所属だけを見る判定へ戻すと `❌ 入れ替わった診断の day11 を想定内として免除している`（33 → 32/33）。

## [PR #389 二十五巡目] Codex 2件（両方採用）

### 1. `check_visualization.py:80` 綴り違いの同じ画像を別物として数えとった（P2・採用）

- 根拠: `find_duplicate_images()` がリンクの**素の文字列**で数えとった。`./screenshots/x.png` と `screenshots/x.png` と `screenshots/../screenshots/x.png` は同じ1枚やのに3種類に見えるので、**同じ画像を綴り違いで3回貼れば「スクショ位置3箇所」を満たしつつ重複0で通る**。既定を FAIL にした七巡目の直しに、この抜け道が残っとった。
- **今の教材では発火せん**（36本のリンクは全部 `./screenshots/...` の1形式）。将来の書き足しで綴りが混ざったら開く穴なので、実害が出る前に塞ぐ。
- 直し: `normalize_image_target()` を足して `posixpath.normpath` で寄せてから数える。外部 URL はそのまま返す。
- 回帰テスト: `DUPLICATE_CASES` に綴り違い3連の case を追加（34件へ）。正規化を外すと `❌ 同じ画像を綴り違いで3回貼っても重複として落ちる: 期待 exit=1 … 実際 exit=0 … 重複0`。

### 2. `shoot-page.mjs:357` 破線の刻みだけで描く線を途中で撮っとった（P2・採用）

- 根拠: Recharts の `<Line>` は線の出現を **`d` を固定したまま** `stroke-dasharray` を伸ばして描く。収束判定の形の文字列に `stroke-dasharray` が入ってへんかったので、初期フレームから形が変わらんように見えて、600ms の窓が動作中に満了する。指摘のとおり `src/app/report/weekly/page.tsx:146` に実物の `<Line>` がある（day23 の週次レポート）。
- 直し: 形の文字列へ `stroke-dasharray` / `stroke-dashoffset` を、属性と inline style の両方から加えた（react-smooth の版で書き込み先が違うため）。
- 回帰テスト: 実ブラウザ検査に fixture を1本追加（6本 → **7本**）。`d` は最初から最終形で、動くのは破線の刻みだけ。窓（600ms）より長い 1500ms 動かして、飾りにならんようにした。属性を見んように戻すと `❌ 破線の刻みだけで描く線を途中で撮っている（stroke-dasharray=62 / 期待 140）`（7 → 6/7）。
