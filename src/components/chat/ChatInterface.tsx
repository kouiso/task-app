...
import type { AIProvider, Message } from '@/types/chat';
  ...
  const handleSend = async () => {
    ...
    enabledProviders.forEach(async (provider) => {
      ...
      setConversations((prev) => ({
        ...prev,
        [provider.id]: [...updatedMessages, { role: 'assistant', content: '', loading: true }],
      }));

      try {
        const response = await sendMessageMutation.mutateAsync({
          provider: provider.id as 'openai' | 'claude' | 'gemini' | 'llama',
        ...
        setConversations((prev) => ({
          ...
          [provider.id]: [
            ...updatedMessages,
            { role: 'assistant', content: response.content, loading: false },
          ],
        }));
      } catch (error) {
        setConversations((prev) => ({
  ...
  return (
    <>
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {isSettingsOpen && (
          <Paper
            sx={{ width: 300, p: 2, borderRight: 1, borderColor: 'divider', overflow: 'auto' }}
          >
          ...
          <Grid container sx={{ height: '100%' }}>
            {enabledProviders.length === 0 ? (
              ...
              enabledProviders.map((provider) => (
                ...
                >
                  <ChatWindow provider={provider} messages={conversations[provider.id] || []} />
                </Grid>
              ))
            )}
          </Grid>
