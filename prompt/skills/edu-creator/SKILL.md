---
name: edu-creator
description: "edu-creator 開発ガイド（実装AI向け）"
---
---
---
...
## 15. 外部ナレッジソース（AIエンジニアリング参照用）

### 参照先

プロンプト改善やAIエンジニアリングの知識が必要な場合は、以下を参照可能：

**パス**: `/home/kouiso/develop/abeja`

| ディレクトリ | 内容 | 活用場面 |
|-------------|------|---------|
| `fujisoft_automate` | 自動化プロジェクト | RPA・自動化パターン |
| `proj-chintai` | 賃貸関連AIプロジェクト | フロントエンド実装例 |
| `proj-chintai-backend` | バックエンド実装例 | API設計・DB設計 |
| `proj-docomo-recommend` | レコメンドシステム | ML推論・推薦アルゴリズム |
| `proj-open-bizrobo-automate` | RPA自動化 | 業務自動化パターン |
| `sompo-sales-ai-demo` | 営業AIデモ | プロダクションAI実装 |

### 活用場面

```
✅ 現場での実装パターンを確認したい時
✅ データサイエンティストの実践例が必要な時
✅ AIエンジニアリングのベストプラクティスを調べる時
✅ プロンプトエンジニアリングの応用例を探す時
✅ 本番環境でのAI運用事例を参照したい時
```

### 注意事項

- 教材の主題（task-app）とは別の**補足的リファレンス**として活用する
- eduAIのチュートリアル生成には直接使用しない
- 実装AI（あなた）がプロンプト改善の参考にするために使用する
...
- **AIエンジニアリングの参照が必要な時は `/home/kouiso/develop/abeja` を確認する**
