...
- 初心者役が挙げた件数: 4 件 / うち反証を生き延びた件数: 2 件
- 外部レビューが挙げた件数: 0 件
- 生き延びた件数（合計）: 2 件 / 反証で消えた件数: 2 件
- 生き延びた2件のうち「2.」は、いったん「成立しない」とした判定を外部レビューで覆したもの。
...
### 2. 行1776 つまずきポイント表が、読者のどこにも存在しない文字列を確認先にしている
...
Step 7 で詳細ダイアログが空のまま出たとき、この行に従って `!!selectedTask` を探す。
しかしその文字列は、読者が書いた `page.tsx` のどこにも無い。実際に条件が書かれているのは
配布済み部品 `src/component/task/task-detail-dialog.tsx` の中の `enabled: !!taskId` であり、
この監査では読者が `src/` を読むことを禁じている。つまり読者は、表が指す場所へ到達できない。

**教材のどこが足りないか**

`selectedTask` が prop `taskId` として渡ることは day13:1088 に書いてあるが、
表の側は名前が変わることに触れていない。前日 Day 12 の `enabled: !!selectedProject` は
読者自身が書いた行なので探せたが、Day 13 の該当行は読者が書いていない。
同じ形に見えて、探せるかどうかが逆になっている。

**反証側が確かめた根拠**

この件は、いったん「成立しない」と判定した。判定を覆した理由を含めて、当時の反証をそのまま残す。
...
【この反証が足りなかった点 — 判定を覆す】
反証は「読者は追える」と述べたが、追えることと、表の指示どおりに探して見つかることは別である。
表は `!!selectedTask` を確認しろと書いており、読者の手元のファイルにその文字列は存在しない。
`selectedTask` が本文に10回出ることも、`enabled` を前日に書いたことも、
`!!selectedTask` という探索対象を手元で見つけられるようにはしない。
よって survives: true。表の確認先を、読者が実際に開ける場所の記述へ直す必要がある。
...
現物確認（material/30days-curriculum/day13_タスク一覧画面.md）。
